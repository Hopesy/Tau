...
public sealed class GeminiCliOAuthProvider : IOAuthProvider
{
    private const string ClientId = "681255809395-oo8ft2oprdrnp9e3aqf6av3hmdib135j.apps.googleusercontent.com";
    ...
    private const int CallbackPort = 8085;
    ...
    public async Task<OAuthCredentials> RefreshTokenAsync(OAuthCredentials credentials, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException($"Google Cloud token refresh failed: {response.StatusCode} {body}");
        }
