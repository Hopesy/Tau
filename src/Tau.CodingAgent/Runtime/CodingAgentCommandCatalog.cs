...
public static class CodingAgentCommandCatalog
{
    ...
    [
        ...
        new("/settings", "/settings [current|path|select]", "Show current settings file, effective settings summary, or open the interactive settings selector"),
        new("/theme", "/theme [current|list|select|set|clear] [name]", "Show, list, interactively select, or persist the current color theme"),
        ...
        new("/metadata", "/metadata [entry-id]", "Inspect JSONL session metadata or a specific tree entry"),
        ...
        new("/model", "/model [current|select [search]|provider/model|model] or /model <provider> <model>", "Show, interactively select, or set the current model"),
        ...
        new("/scoped-models", "/scoped-models [current|select|set|add|remove|clear|all] [provider/model ...]", "Show or configure the persisted model cycle scope"),
        ...
        new("/auth", "/auth [current|select|provider]", "Show provider auth status"),
        new("/login", "/login [select|provider]", "Login to a provider or choose one interactively"),
        new("/logout", "/logout [select|provider]", "Logout from a provider or choose one interactively"),
        ...
        new("/thinking", "/thinking [current|select|cycle|off|minimal|low|medium|high|xhigh]", "Show, select, or set the thinking (reasoning) level"),
