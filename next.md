...
- [x] session lifecycle（session delete / export JSON download / export.html / export.md / export.jsonl 线性 transcript / import.jsonl 线性 transcript roundtrip / `.jsonl` 前端文件上传识别 / JSONL import content-type 校验 / `application/problem+json` 稳定错误 code+line / CodingAgent JSONL session 只读 preview endpoint `/api/sessions/import.coding-agent-jsonl/preview` / preview-derived conservative import endpoint `/api/sessions/import.coding-agent-jsonl` / clone / search by title / import file upload / title rename / last-opened session restore baseline / clear messages 端点；CodingAgent import 会持久化保守 WebChat DTO，不替换 WebChatStore，也不是完整 branch/tree parity）
...
- [x] 当前本地项目级测试基线（`Tau.Ai.Tests` 205、`Tau.CodingAgent.Tests` 346、`Tau.WebUi.Tests` 29、`Tau.Agent.Tests` 76、`Tau.Pods.Tests` 66、`Tau.Tui.Tests` 110；本轮 targeted tests 已串行通过，全仓 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-dotnet.ps1` 已通过）
