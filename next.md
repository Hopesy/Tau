...
- [~] 消息区 / 状态区（已补库层 `TuiMessageArea` 和 `TuiStatusBar`：role prefix、wrap、bottom-anchored visible transcript、left/right status segment 与窄宽截断；仍未接入完整 terminal host、viewport/scrollback 或 CodingAgent 主屏）
...
- [x] session lifecycle（session delete / export JSON download / export.html / export.md / export.jsonl 线性 transcript / import.jsonl 线性 transcript roundtrip / `.jsonl` 前端文件上传识别 / JSONL import content-type 校验 / `application/problem+json` 稳定错误 code+line / clone / search by title / import file upload / title rename / last-opened session restore baseline / clear messages 端点）
...
- [~] 更高层 delegation flow 与端到端测试（已补 `MomLocalDelegationFlow`，统一 `Worker` 和 `--once` 的 event -> inbox request -> runner -> outbox/status/log/archive 本地流程，并用 fake runner e2e 固定 provider/model normalize、attachment staging 和 inbox cleanup；真实 Slack smoke、真实 Docker smoke 和多消息 session sync 仍缺）
...
- [x] 当前本地项目级测试基线（`Tau.Ai.Tests` 196、`Tau.CodingAgent.Tests` 336、`Tau.WebUi.Tests` 17、`Tau.Agent.Tests` 68、`Tau.Pods.Tests` 55、`Tau.Tui.Tests` 92；`verify-dotnet.ps1 -SkipRestore` 已串行通过）
