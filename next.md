...
- [~] JSONL tree navigator / richer session metadata（已补命令行 tree filter/search：`default/no-tools/user-only/labeled-only/all`、settings `treeFilterMode`、`--search query` 与 `--label-time`；已补 `/tree --interactive`（`-i`）j/k/↑/↓/g/G/Enter/q/Esc 的交互式 navigator baseline，复用 `IConsoleKeyReader`，输出 `selected entry <id>`/`tree navigator cancelled` 状态；已补 overlay `/` search（incremental filter + Backspace + Esc clear）、`f` filter cycling（default→no-tools→user-only→labeled-only→all→default）、`n`/`N` next/prev match、filter/search 切换时保持 selected entry id 稳定；仍缺 fold/select 和完整 metadata）
