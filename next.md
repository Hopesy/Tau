...
- [~] secret 持久化边界和脱敏规则（已补默认 `./.tau/auth.json` / `./.tau/models.json` / JSONL session 本地状态忽略、`auth.json` Unix 0600 写入、OAuth metadata 保留字段过滤、auth status 不回显密钥、models.json credential header 状态识别、command-backed `apiKey` 状态检查不执行 `!command` 的回归、HTML transcript 导出（`/export` / `/share`）默认对常见 AWS / GitHub / Slack / Anthropic / OpenAI / Bearer / JWT secret 模式做 `[redacted]` 替换（`TAU_CODING_AGENT_REDACT_SECRETS=0` 可关闭），以及 `TauSecretRedactor` 提升到 `Tau.Ai` 并被 `Tau.WebUi GET /api/sessions/{id}/export.html` 使用（`TAU_WEBUI_REDACT_SECRETS=0` 可关闭）；已补 `JsonlTauLogSink` runtime event log 默认 pattern-based 脱敏 baseline（`TAU_LOG_REDACT_SECRETS=0` 可在 sink 创建时关闭，覆盖常见 secret pattern 的 category/event/field value）；已补 `JsonlSecretRedactor` 通用 JSONL string value redaction foundation，并让 Mom channel `log.jsonl` / `last_prompt.jsonl` 默认复用该能力（`TAU_MOM_REDACT_SECRETS=0` 可关闭）；仍需继续梳理 CodingAgent JSONL session tree、WebUi JSONL export/import、field key 和非标准 secret pattern 的边界）
...
- [x] 当前本地项目级测试基线（`Tau.Ai.Tests` 204、`Tau.CodingAgent.Tests` 342、`Tau.WebUi.Tests` 29、`Tau.Agent.Tests` 76、`Tau.Pods.Tests` 65、`Tau.Tui.Tests` 110；本轮 targeted tests 已串行通过，最终 `verify-dotnet.ps1 -SkipRestore` 仍需主控收尾跑全链）
...
- [~] 可观测性：provider 调用、auth、tool execution、session / delegation / pod probe 的最小日志（已补 `Tau.Ai.Observability.ITauLogSink`/`JsonlTauLogSink`/`NullTauLogSink` baseline，默认写 `./.tau/log.jsonl`，可被 `TAU_LOG_FILE` 覆盖或 `TAU_LOG_DISABLED=1` 关闭；`JsonlTauLogSink` 当前默认对 runtime event log 做 pattern-based redaction，但只覆盖该事件日志 sink 的 category/event/field value，不代表其它 JSONL 文件自动脱敏；`RuntimeCodingAgentRunner.RunAsync` 已 emit `agent/run.start|run.end|run.cancel|run.error` 含 provider/model/inputBytes/elapsedMs/error 字段；仍缺 auth status、tool execution、session 持久化、Mom delegation、Pod probe 几个面的事件）
