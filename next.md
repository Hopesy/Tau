...
- [~] session lifecycle（已补 `/new`、`/session` tree stats、`/metadata [entry-id]` session/entry inspector、估算 token/context usage、auto-compaction threshold budget、retry policy display、JSONL `cwd/parentSession` metadata、`/name`、`/tree` 过滤/搜索模式与 label timestamp、`/tree --interactive` navigator、`/label`、`/fork`、`/fork <entry-id> --summarize [instructions]`、`/clone`、`/resume`、`.jsonl` export/import；`/fork --summarize` 会写 JSONL `branch_summary`，恢复 branch 时作为 context message 注入 runtime；仍缺完整 TUI metadata inspector、自动 branch switching summarization hooks 和上游 TreeSelector parity）
...
- [x] session lifecycle（session delete / export JSON download / export.html / export.md / export.jsonl 线性 transcript / import.jsonl 线性 transcript roundtrip / `.jsonl` 前端文件上传识别 / JSONL import content-type 校验 / `application/problem+json` 稳定错误 code+line / WebUi JSONL export/import 默认 string-value secret redaction / CodingAgent JSONL session 只读 preview endpoint `/api/sessions/import.coding-agent-jsonl/preview` / preview-derived conservative import endpoint `/api/sessions/import.coding-agent-jsonl` / CodingAgent JSONL preview/import 默认 string-value secret redaction / clone / search by title / import file upload / title rename / last-opened session restore baseline / clear messages 端点；CodingAgent import 会持久化保守 WebChat DTO，不替换 WebChatStore，也不是完整 branch/tree parity）
...
- [~] message / runtime flow（已补最小 `log.jsonl` channel history 注入、本地 request/result 写回、`context.json` runtime messages、same-workdir provider/model/session name carry-over、`last_prompt.jsonl` prompt debug snapshot、`log.jsonl` / `last_prompt.jsonl` 默认 secret redaction baseline、`status.json` runtime 状态、本地 busy-state guard、Slack-compatible envelope、Slack event mapper、Slack Socket Mode transport seam、Slack startup backfill seam、channel processor busy/stop/typing/thread response seam、true cancellable stop seam、Slack Web API responder seam、Slack private file download seam 与 per-channel queue seam，并已有 fake runner 覆盖本地连续消息路径；仍缺真实 Slack session sync、真实 Slack smoke 下的多消息 flow 和更高层 delegation e2e）
...
- [x] vLLM command planner baseline（`PodVllmCommandPlanner` 本地生成 serve/systemd/metadata/remote command plan，固定 model cache path convention、deployment name、port、served model name、env、extra args 和 shell quoting；`vllm plan [--json] [path] <id> <model> [name]` 已接入 CLI 只打印计划，默认文本输出，`--json` 输出 machine-readable plan；不执行 SSH，不启动 systemd unit，也不是 rollout/orchestration）
- [~] 真正的 CLI 运维命令体系（已补 `health/deploy/stop/restart/logs/deployments`、`model list/pull/remove/status` model lifecycle baseline、`vllm plan` plan-only CLI baseline 与 `--json` 机器可读输出、pod-level `modelsPath`、长耗时 `pull` 的 SSH keepalive、可省略 path 的 target-command 参数解析、SSH lifecycle metadata 命令转义、SSH exec `ArgumentList` argv hardening、本地 ssh 启动失败/runner 异常/cancellation 结构化返回、vLLM command planner baseline 和 CLI 参数回归测试；仍缺真实 HF download/SSH smoke、vLLM orchestration、远端模型 snapshot 解析、systemd install/start/status、失败回滚、更完整远端 transport hardening 和真实运维 smoke）
...
- [x] 当前本地项目级测试基线（`Tau.Ai.Tests` 205、`Tau.CodingAgent.Tests` 346、`Tau.WebUi.Tests` 32、`Tau.Agent.Tests` 76、`Tau.Pods.Tests` 66、`Tau.Tui.Tests` 110；本轮 targeted tests 已串行通过，全仓 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-dotnet.ps1` 已通过）
