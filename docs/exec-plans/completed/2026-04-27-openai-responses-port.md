...
- 风险：把 `openai-responses` / `openai-codex-responses` 从 `OpenAiCompatibleProvider` 切走后，依赖现有 chat-completions 兜底语义的调用链回归。
  - 缓解方式：仅替换这两个 api 的 builtin 注册，`OpenAiCompatibleProvider` 类本身不动；provider 测试加断言确认这两个 api 的实例类型已切换，旧 `mistral` / `azure-openai-responses` 路径不变。
...
- 手工检查：
  - `OpenAiResponsesProviderTests` 显式断言 `openai-responses` 与 `openai-codex-responses` 解析出的实例类型分别是 `OpenAiResponsesProvider` 与 `OpenAiCodexResponsesProvider`
...
- [x] 1. 共享层（converter + stream event 翻译 + id 编解码 + JWT decoder）
- [x] 2. `OpenAiResponsesProvider` + 注册切换 + 测试
- [x] 3. `OpenAiCodexResponsesProvider` + 测试
- [x] 4. `next.md` / `QUALITY_SCORE` / `ARCHITECTURE` 同步
- [x] 5. `dotnet build/test` 全绿 + history 落地
...
- 2026-04-28：完成 SSE 版本落地并保留 out-of-scope 边界。`openai-responses` 与 `openai-codex-responses` 已切到专用 provider；Codex 内建模型 base URL 对齐 `https://chatgpt.com/backend-api`；验证集中在 StubHandler 驱动的 payload / stream event / header / retry，不把真实 OpenAI 网络当本地门禁。
