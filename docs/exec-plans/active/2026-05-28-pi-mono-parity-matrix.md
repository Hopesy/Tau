...
| --- | --- | --- | --- | --- |
| AI + Agent | `packages/ai`, `packages/agent` | `src/Tau.Ai`, `src/Tau.Agent`, tests | partial | Capability inventory merged from local scan. Known final blockers include provider e2e, auth refresh/registration, broader config UX, Agent facade/proxy/event/correlation closure. |
...
| Agent stream proxy | `packages/agent/src/proxy.ts` reconstructs partial assistant messages from stripped proxy events via `/api/stream` | No direct Tau equivalent found in `src/Tau.Agent` | missing | Needs either a .NET proxy stream function or an explicit non-goal decision; current WebUi/CodingAgent direct runner path does not prove proxy parity. |
