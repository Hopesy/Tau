...
| Agent runtime loop, facade and tool events | `packages/agent/src/agent.ts`, `agent-loop.ts`, `types.ts` | `src/Tau.Agent/Agent.cs`, `Runtime/AgentRuntime.cs`, `ToolExecutor.cs`, `Abstractions/AgentEvents.cs`, `tests/Tau.Agent.Tests/AgentFacadeTests.cs`, queue/trace tests | partial | Tau now has a high-level `Agent` facade baseline with prompt/continue/subscribe/queue/wait/reset tests and runtime tool-result message lifecycle. Remaining gaps include exact `agent_end.messages`, `turn_end.message/toolResults`, richer tool update payloads, schema validation and full error/cancel edge parity. |
...
| --- | --- | --- | --- |
| `packages/agent/src/agent.ts` | `src/Tau.Agent/Agent.cs`, `AgentState.cs`, `AgentRuntime.cs`, `tests/Tau.Agent.Tests/AgentFacadeTests.cs` | partial | `Agent` baseline now covers `AgentOptions`, state exposure, `Subscribe`, `PromptAsync`, `ContinueAsync`, steering/follow-up queues, queue clear, `HasQueuedMessages`, `Abort`, `WaitForIdleAsync`, `Reset`, prompt message lifecycle and listener settlement. Still missing exact synthetic assistant failure message shape, abort signal plumbing parity, session/transport/thinking budget pass-through at facade level and every upstream state mutator edge. |
| `packages/agent/src/agent-loop.ts` | `src/Tau.Agent/Runtime/AgentRuntime.cs`, `ContextTransformer.cs`, `ToolExecutor.cs`, `AgentEvents.cs`, queue/tool trace tests | partial | Tau has double loop, context transform/convert, sequential/parallel tools, before/after interceptors, runtime log trace and tool-result message lifecycle. Still missing `EventStream<AgentEvent, AgentMessage[]>` wrapper shape, `agent_end.messages`, `turn_end.message/toolResults`, rich tool update payloads, schema validation, error-as-tool-result for prepare/execute/after exceptions and exact parallel event timing. |
| `packages/agent/src/types.ts` | `AgentLoopConfig.cs`, `AgentState.cs`, `IAgentTool.cs`, `IToolInterceptor.cs`, `AgentEvents.cs`, `AgentQueueMode.cs` | partial | Core .NET records/interfaces exist and `AgentState` now includes system prompt/model/tools plus runtime fields. TypeScript declaration-merge custom message semantics are intentionally .NET-native; event payload and tool update/detail contracts still need file-level closure. |
...
| `packages/agent/src/index.ts` | `src/Tau.Agent/Tau.Agent.csproj` public assembly surface | partial | Public assembly now exposes facade/runtime/types/proxy by namespace instead of a TypeScript barrel. Need compile-sample/API surface test and final decision for any TypeScript export without Tau-native equivalent. |
