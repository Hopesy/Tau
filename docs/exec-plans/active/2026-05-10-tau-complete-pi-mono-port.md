...
- `/session` 同时报告 flat stats、估算 token/context usage、auto-compaction threshold budget 和 tree file/leaf/entry/message/branch/label 信息；`/tree [max entries] [default|no-tools|user-only|labeled-only|all] [--label-time] [--search query]` 输出短 id、parent、entry 摘要、label、label timestamp、当前 branch 与 leaf 标记，并按上游 tree selector 的核心过滤语义隐藏 bookkeeping / tool-only assistant / tool result；`--search` 会按空格分词匹配 entry text、label 和基础 metadata；未显式传 filter 时读取 settings `treeFilterMode`，无效值回退 `default`；`/label <entry-id> [label | clear]` 追加 label change；`/fork <entry-id>` 从历史 entry 切出新 branch 并恢复当前 runner messages；`/resume [latest | path.jsonl]` 恢复 JSONL session。
