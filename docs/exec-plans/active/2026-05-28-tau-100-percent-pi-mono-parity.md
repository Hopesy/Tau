...
- 当前 CodingAgent package/changelog/telemetry 局部 baseline 覆盖 source 持久化、列表/config 摘要、本地 package resource discovery/filter、上游式 extension entry discovery/status、JavaScript/TypeScript `registerCommand` limited runtime、npm/git install/update/remove execution、startup version state 和 install telemetry runtime。仍未关闭 interactive config selector、完整 `@mariozechner/jiti` import/alias/virtualModules parity、`registerTool`、flags/shortcuts/events lifecycle、custom tool wrappers、最终 `pi` package/bin identity、真实 npm/git network smoke、真实 package consumer smoke 或真实 install telemetry e2e。
