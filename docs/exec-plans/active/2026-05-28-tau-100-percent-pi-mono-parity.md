...
- 当前 CodingAgent package/changelog/telemetry 局部 baseline 覆盖 source 持久化、列表/config 摘要、本地 package resource discovery/filter、上游式 extension entry discovery/status、JavaScript/TypeScript `registerCommand` limited runtime、JavaScript/TypeScript `registerTool` limited runtime、`prepareArguments` 参数准备 baseline、`tool_call` block/input mutation 与 `tool_result` rewrite hook baseline、npm/git install/update/remove execution、startup version state 和 install telemetry runtime。仍未关闭 interactive config selector、完整 `@mariozechner/jiti` import/alias/virtualModules parity、renderer/details-rich content、flags/shortcuts/events lifecycle、custom tool migration wrappers、运行中 runner tool hot-swap、最终 `pi` package/bin identity、真实 npm/git network smoke、真实 package consumer smoke 或真实 install telemetry e2e。
...
- 完整 jiti import/alias/virtualModules parity、custom tools/events/flags/shortcuts、theme watcher、resource selector、full diagnostics。
