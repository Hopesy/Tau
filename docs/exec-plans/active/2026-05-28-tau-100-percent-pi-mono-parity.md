...
- [ ] Phase 3：product runtime parity。
- [ ] Phase 4：external e2e closure。
- [~] Phase 5：release/CI/install delivery parity（current-RID artifact baseline 已落地：`scripts/build-release-artifacts.ps1` 会 publish `pi`、`tau-ai` / `pi-ai`、`mom`、`pi-pods`、`tau-web-ui` 到 `artifacts/tau-<rid>/`，生成 wrapper 与 `manifest.json`；`scripts/smoke-release-artifacts.ps1` 会在 artifact 目录验证 AI CLI list、`pi` RPC `get_state`、Pods help、WebUi HTTP API 和 Mom `--once`。PowerShell no-env wrapper baseline 也已落地：`scripts/invoke-no-env.ps1` 只对子进程清除 provider/AWS/Azure/HF/Slack/Tau auth/config env，并可把 Tau auth/models/session/settings/log 指到临时目录；`scripts/verify-no-env.ps1` 会在该隔离环境运行项目验证并可加 `-RunSmoke` 覆盖 `tau-ai list` 与 CodingAgent RPC `get_state`。本轮新增 `scripts/pi-test.ps1`，把上游 `pi-test.sh --no-env` 的“直达 CodingAgent CLI + 可选 no-env”职责落成 PowerShell wrapper，`verify-no-env.ps1 -RunSmoke` 的 CodingAgent RPC smoke 已改由它执行。当前又新增 `scripts/package-release-artifacts.ps1`，为 current-RID artifact 生成 `artifacts/releases/tau-<rid>.zip` 并在 clean temp 解压后复用 artifact smoke 验证 extracted copy。现在新增 `.github/workflows/tau-ci.yml`，让 GitHub Actions Windows runner 复用同一 no-env smoke、Release artifact build、archive package/extract smoke，并上传 `tau-win-x64.zip` workflow artifact。仍缺全平台 archive matrix、Unix tar.gz parity、exact Unix wrapper/auth-backup parity、version/changelog/tag/publish automation 与真实外部 e2e release smoke）。
- [ ] Phase 6：final 100% acceptance。
