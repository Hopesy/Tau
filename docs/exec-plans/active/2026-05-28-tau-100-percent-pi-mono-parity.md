...
- [~] 2026-06-08：继续 Phase 2 `Pods config path/env compatibility`。对照上游 `packages/pods/src/config.ts`，Tau 现在在未显式传 config path 时支持 `PI_CONFIG_DIR`，默认读取/写入 `<PI_CONFIG_DIR>/pods.json`；显式 `--config path` 和旧 positional config path 继续优先于 env default；`help` 增加 env/default config 说明。已通过 `dotnet build src\Tau.Pods\Tau.Pods.csproj --no-restore --verbosity minimal`、focused `PodsCli|Config` gate 87/87、完整 `Tau.Pods.Tests` 184/184 和项目级 `verify-dotnet.ps1 -SkipRestore`（Ai 280、Agent 119、Tui 251、CodingAgent 438、WebUi 44、Pods 184）。该切片只关闭 `PI_CONFIG_DIR -> pods.json` 子合同，仍不关闭无 env 时上游 `~/.pi/pods.json` 默认、`loadConfig()` missing-file 返回 `{ pods: {} }` 的错误语义差异、upstream record-shaped schema/state migration、真实 remote e2e 或 Pods final `verified` 状态。
- [ ] Phase 3：product runtime parity。
- [ ] Phase 4：external e2e closure。
...
- 2026-06-08：决定把 `Tau.Pods` config path/env 的第一刀限定为 `PI_CONFIG_DIR` env override，而不是直接把 Tau 默认配置从 `tau.pods.json` 改成 `~/.pi/pods.json`。原因是当前 Tau 大量本地 CLI/test workflow 仍依赖 repo-local default，直接切换会把 path default、missing-config 语义和 schema migration 混成一个难审查提交；本轮先固定可验证的上游 env override 和显式 config precedence，剩余 no-env default、missing config `{ pods: {} }`、record-shaped schema migration 后续单独收口。
