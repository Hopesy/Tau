...
- [~] 2026-06-08：继续 Phase 2 `Pods deploy-time startup watcher` 子切片。对照上游 `startModel()` 启动后 tail `~/.vllm_logs/<name>.log` 并等待 `Application startup complete` / failure marker 的合同，Tau 现在让 `vllm deploy` / top-level `start` 在 remote deploy command 成功后先运行 `PodVllmOrchestrationService.WatchStartupAsync`，结构化输出 `startupWatch` JSON/text；watcher 扫描同一上游日志路径，ready 后继续现有 `/health` readiness，失败 marker 返回 `failureKind=startup-failed` 并触发 rollback，CLI 不写入 failed deployment configured model state。当前已通过 Pods build、focused `DeployAsync|VllmDeploy|Start_WithName|Start_WithExisting` gate 34/34、完整 `Tau.Pods.Tests` 214/214 和项目级 `verify-dotnet.ps1 -SkipRestore`（Ai 280、Agent 119、Tui 251、CodingAgent 438、WebUi 44、Pods 214）。该切片只关闭 deploy-time startup marker wait / failure rollback / no-writeback baseline；exact `model_run.sh` wrapper、pseudo-TTY `script`、真实 SSH/HF/GPU/vLLM smoke、多版本 rollback 和 Pods final `verified` 状态仍保持 open。
- [~] 2026-06-08：继续 Phase 2 `Pods model runner wrapper` 子切片。对照上游 `packages/pods/scripts/model_run.sh` 与 `commands/models.ts` 的 runner upload + pseudo-TTY wrapper 启动合同，Tau 现在生成并上传 `~/.tau_pods/model_run_<name>.sh` 与 `~/.tau_pods/model_wrapper_<name>.sh`，systemd 与 fallback 都启动同一 wrapper；runner 保留 `Model runner exiting with code`，wrapper 追加 `Script exited with code`，wrapper 通过 `script -q -f -c ... ~/.vllm_logs/<name>.log` 写 upstream log path，systemd 不再重复 append wrapper stdout/stderr；plan/deploy text/JSON 和 metadata 暴露 runner/wrapper script path 与 `usesPseudoTtyWrapper`，stop/rollback cleanup 会删除 runner/wrapper 脚本。当前已通过 Pods build、focused `PodVllmCommandPlannerTests|DeployAsync|VllmDeploy|VllmPlan|VllmRollback` gate 65/65、完整 `Tau.Pods.Tests` 214/214、`git diff --check` 和项目级 `verify-dotnet.ps1 -SkipRestore`（Ai 280、Agent 119、Tui 251、CodingAgent 438、WebUi 44、Pods 214）。该切片本身只关闭本地 runner/wrapper/pseudo-TTY 合同；HF download / PI API key command baseline 已由后续独立切片收口，save-before-watch/delete-config exact ordering、真实 SSH/HF/GPU/vLLM smoke、多版本 rollback 和 Pods final `verified` 状态仍保持 open。
- [~] 2026-06-08：继续 Phase 2 `Pods model runner HF download / PI API key` 子切片。对照上游 `packages/pods/scripts/model_run.sh` 的启动脚本合同，Tau 生成的 `model_run_<name>.sh` 现在会在 runner header 后执行 `HF_HUB_ENABLE_HF_TRANSFER=1 hf download "$MODEL_ID"`，失败时输出稳定 `ERROR: Failed to download model` 并退出，成功时输出 `Model download complete`；Tau-planned `vllm serve` 命令现在固定带 `--api-key "$PI_API_KEY"`，再追加 served model name 和额外 vLLM args。当前已通过 focused `PodVllmCommandPlannerTests|DeployAsync|VllmDeploy|VllmPlan|VllmRollback` gate 65/65、完整 `Tau.Pods.Tests` 214/214、`git diff --check` 和项目级 `verify-dotnet.ps1 -SkipRestore`（Ai 280、Agent 119、Tui 251、CodingAgent 438、WebUi 44、Pods 214）；该切片只关闭本地 runner HF download / PI API key command contract，不证明真实远端 HF download、PI API key 环境、SSH/GPU/vLLM startup smoke、save-before-watch/delete-config exact ordering、多版本 rollback 或 Pods final `verified` 状态。
- [ ] Phase 3：product runtime parity。
- [ ] Phase 4：external e2e closure。
