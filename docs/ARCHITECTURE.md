...
- JSONL 行级脱敏从 runtime log 专用实现扩成 `JsonlSecretRedactor` foundation：JSON 行会递归只处理 string value，不改 object key，number/bool/null 保留，invalid JSON fallback 到整行 pattern redaction；`Tau.Mom` 已复用该 helper 处理 `log.jsonl` 和 `last_prompt.jsonl`，但 CodingAgent session JSONL、WebUi JSONL export/import、field key 和非标准 secret pattern 仍需后续单独收口。
...
当前这仍是 foundation + 若干 CodingAgent selector / app action 接线，已经能用 targeted tests 固定组件/render/selector/session/ANSI sink/message-status/scrollback/transcript viewport foundation、Ctrl+P/Ctrl+Shift+P model cycle 行为和 Ctrl+L model selector 行为，并已把主题、settings、scoped models、model select、auth status、`/login` OAuth provider 选择、`/logout` OAuth provider 选择和 `/thinking select` thinking level 选择接回真实命令；但 `TuiScrollbackBuffer` / `TuiTranscriptViewport` 尚未接管 `InteractiveConsoleSession`、`TuiAnsiRenderSurface`、CodingAgent 主屏或 selector overlay，完整上游 TUI host、overlay compositing、硬件 cursor、theme rendering 和完整 OAuth login dialog/session / resource selector UI 仍是后续切片。
...
- `POST /api/sessions/import.coding-agent-jsonl` 提供 preview-derived conservative import：复用同一 parser，把 timeline message 保守转换成 WebChat message 并通过现有 `ImportSession` 持久化；user 保持 user，assistant/toolResult 等非 user 统一导入为 assistant，tool call/result/thinking/image 只写入可审计文本标记，避免生成空消息。该 endpoint 不保留 CodingAgent branch/tree 结构，也不替换 `WebChatStore`
...
- 当前 WebUi session 仍是 WebChatStore DTO；虽已有 WebUi-local 线性 JSONL transcript export/import roundtrip、CodingAgent JSONL 只读 preview 和 conservative import，但还不是 CodingAgent JSONL branch/session-tree 导入或持久化语义
