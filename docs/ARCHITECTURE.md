...
- CLI initial prompt / `@file` baseline：`Program.cs` 解析上游式 print/RPC/theme/context 相关参数、普通 CLI messages 和 `@file` 参数；非 RPC redirected stdin 会按上游 print-mode 语义进入初始 prompt，空白 stdin 不触发 print，RPC 模式拒绝 `@file` 以保持 JSONL stdin 协议纯净。`CodingAgentInitialMessageBuilder` 会把 stdin、`@file` 文本、PNG/JPEG/GIF/WebP 图片和第一条 CLI message 合并为 initial prompt；print mode 与 interactive startup 在含图片时走 `ContentBlock` runner 路径，`images.blockImages` 会阻断图片发送并插入文本 notice。当前这是 CLI 初始输入文件/图片 ingestion baseline，不代表 clipboard image paste、Photon resize/convert/EXIF、provider vision e2e 或完整 TUI attachment/editor path 已完成。
