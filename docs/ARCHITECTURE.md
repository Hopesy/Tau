...
- `TuiSettingsList` 提供上游 `components/settings-list.ts` 的库层 foundation：label/value 对齐、description wrapping、可选搜索、Enter/Space value cycle、submenu delegate / done callback、Esc/Ctrl-C cancel、滚动提示和宽度截断；当前尚未替换 `Tau.CodingAgent` 的产品级 `/settings` selector
