...
当前这仍是 foundation + 若干 CodingAgent selector / app action 接线，已经能用 targeted tests 固定组件/render/selector/session/ANSI sink/message-status foundation、Ctrl+P/Ctrl+Shift+P model cycle 行为和 Ctrl+L model selector 行为，并已把主题、settings、scoped models、model select、auth status、`/login` OAuth provider 选择、`/logout` OAuth provider 选择和 `/thinking select` thinking level 选择接回真实命令；但尚未接入完整上游 TUI host、overlay compositing、viewport/scrollback 管理、硬件 cursor、theme rendering，也没有把完整 OAuth login dialog/session / resource selector UI 接回 `Tau.CodingAgent`。
...
- 当前 WebUi session 仍是 WebChatStore DTO；虽已有 WebUi-local 线性 JSONL transcript export/import roundtrip，但还不是 CodingAgent JSONL branch/session-tree 语义
...
- 还没有真实模型进程编排、镜像/服务管理、vLLM runner 编排和 rollout 状态机
