...
- session lifecycle：`/new` 清空当前 runtime messages 和 display name，并在 tree 中追加新的 root marker；`/session` 输出 flat stats、估算 token/context usage、auto-compaction threshold budget、tree file/leaf/entry/message/branch/label 信息；`/tree [max entries]` 输出当前 JSONL tree 的短 id / parent / 当前 branch 标记 / label；`/label <entry-id> [label | clear]` 以 append-only label entry 标记历史节点；`/fork <entry-id>` 从历史 entry 切出新 branch 并恢复该 branch messages；`/resume [latest | path.jsonl]` 恢复 JSONL session；当前仍未完成上游 interactive tree navigator 和更完整 session metadata
