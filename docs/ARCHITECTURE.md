...
当前这仍是 foundation + 若干 CodingAgent selector / app action 接线，已经能用 targeted tests 固定组件/render/selector/session/ANSI sink、Ctrl+P/Ctrl+Shift+P model cycle 行为和 Ctrl+L model selector 行为，并已把主题、settings、scoped models、model select、auth status、`/login` OAuth provider 选择、`/logout` OAuth provider 选择和 `/thinking select` thinking level 选择接回真实命令；但尚未接入完整上游 TUI host、overlay compositing、message/status area、viewport/scrollback 管理、硬件 cursor、theme rendering，也没有把完整 OAuth login dialog/session / resource selector UI 接回 `Tau.CodingAgent`。
...
- session delete/export/import 已接入，前端会记住 last-opened session 并在 reload 时恢复仍存在的会话；`GET /api/sessions/{id}/export.jsonl` 会把当前 WebChat DTO 导出为 WebUi-local 线性 JSONL transcript，首行 `type=session` header，后续 `type=message` entries 使用稳定 `message-000001` id 和线性 `parentId`；`POST /api/sessions/import.jsonl` 会校验同一线性 transcript 并导回新的 WebChat session
