...
- `examples/Tau.Agent.ConsoleExample` 和 `examples/Tau.Agent.HttpExample` 证明 Tau 可以作为源码示例的普通 .NET Agent 应用底座消费：两个示例都使用 `Tau.Ai.Providers.Faux`、delegate tool、`InMemoryAgentSessionStore` 和 runtime log sink，并通过 `scripts/verify-agent-platform-examples.ps1` 做本地 smoke；该 smoke 已接入 `scripts/verify-dotnet.ps1 -RunSmoke`。
- `scripts/verify-agent-package-consumer.ps1` 证明 Tau 可以作为 NuGet-style 本地包被外部项目消费：临时 consumer 只引用 `Tau.Agent` package，通过传递依赖使用 `Tau.Ai.Providers.Faux`，并运行 `AgentApplication`、delegate tool、session store 和 runtime log 回合。

当前仍不是完整上游 Agent package parity：`agent_end.messages`、`turn_end.message/toolResults`、tool update partial result、schema validation、tool exception -> error tool result、provider run + tool execution runtime trace、facade/runtime/proxy/event public API compile-sample、high-level facade 的 stream failure / cancellation synthetic assistant message、low-level `AgentRuntime.RunStream(...)` / `EventStream<AgentEvent, ChatMessage[]>` wrapper、parallel tool start/update timing、assistant stream cancellation terminal event、aborted `ErrorEvent` stop reason preservation、tool cancellation cleanup、parallel sibling cancellation result preservation、`TransformContext` cancellation、第一版 Agent platform API / examples 和本地 package consumer smoke 都已落地；剩余更大范围缺口现在转为真实 provider/OAuth e2e、provider/auth missing UX、真实 package registry 发布/签名/溯源演练、`index`/export 形状决策和真实 proxy/e2e 验证。
...
- CLI initial prompt / `@file` baseline：`Program.cs` 解析上游式 print/RPC/theme/context 相关参数、普通 CLI messages 和 `@file` 参数；非 RPC redirected stdin 会按上游 print-mode 语义进入初始 prompt，空白 stdin 不触发 print，RPC 模式拒绝 `@file` 以保持 JSONL stdin 协议纯净。`CodingAgentInitialMessageBuilder` 会把 stdin、`@file` 文本、PNG/JPEG/GIF/WebP 图片和第一条 CLI message 合并为 initial prompt；print mode 与 interactive startup 在含图片时走 `ContentBlock` runner 路径，`images.blockImages` 会阻断图片发送并插入文本 notice。`CodingAgentImagePreprocessor` 被 CLI `@file` 与 `read_file` 共用，当前覆盖本地 PNG resize 到上游默认 2000x2000 / 4.5MiB base64 headroom、尺寸说明和 JPEG/WebP EXIF orientation 尺寸解析，并由 `images.autoResize` 控制 `ReadFileTool` 启动行为。当前仍不是完整 image ingestion parity：desktop/OSC52 image clipboard、完整 Photon JPEG/GIF/WebP resize/convert quality parity、terminal PNG conversion、provider vision e2e 和完整 TUI attachment/editor path 仍是后续切片。
- RPC mode baseline：`--mode rpc` 走严格 LF-delimited JSONL stdin/stdout，`CodingAgentRpcHost` 直接复用 runner / flat session / JSONL tree / settings seam，当前覆盖 `prompt`、`steer`、`follow_up`、`abort`、`new_session`、`get_state`、`get_settings`、`update_settings`、`set_model`、`cycle_model`、`get_available_models`、`set_thinking_level`、`cycle_thinking_level`、`set_auto_retry`、`abort_retry`、`bash`、`abort_bash`、`set_steering_mode`、`set_follow_up_mode`、`set_auto_compaction`、`switch_session`、`get_fork_messages`、`compact`、`fork`、`clone`、`get_session_stats`、`get_messages`、`get_commands`、`export_html`、`get_last_assistant_text`、`set_session_name`；`new_session` 会读取可选 `parentSession` 并写入 JSONL header metadata；`get_state` 返回 runner/settings backed `steeringMode`、`followUpMode` 和 `autoCompactionEnabled`；`get_settings` / `update_settings` 读写 Tau 当前已真实生效的 settings 字段，并同步 runner model/thinking/queue mode、retry、auto-compaction 与 theme 设置状态；`bash` 通过可注入 `ICodingAgentShellRunner` 后台执行 shell 命令并返回 output/exitCode/cancelled/truncated，同时输出 `bash_output` chunk 和 `bash_event` lifecycle；`CodingAgentRpcExtensionUiBridge` 提供上游同形 `extension_ui_request` / `extension_ui_response` bridge，覆盖 select/confirm/input/editor/notify/status/widget/title/editor-text。它是 Tau-native headless baseline，不等于完整 TypeScript extension runtime、package-loaded extensions、custom tools、RPC client helper、settings selector UI / theme selector / terminal / packages 等全量配置面和 full command parity
...
- 每个 WebUi session 现在还有独立 artifact state：`WebArtifactStore` 默认持久化到 `output/webui-artifacts.json`，`GET/PUT/DELETE /api/sessions/{id}/artifacts[/{fileName}]` 暴露 CRUD，`POST /api/sessions/{id}/runtime/messages` 固定 sandbox runtime message bridge 的 `artifact-operation list/get/createOrUpdate/delete`、console、file-returned 和 execution complete/error response shape；前端 artifact pane 会列出 artifact、渲染 text/image/html 预览，并为 HTML artifact 注入 `listArtifacts` / `getArtifact` / `createOrUpdateArtifact` / `deleteArtifact` helper。该能力是 server-backed artifact/runtime bridge baseline，不是上游 Lit `ArtifactsPanel`、`SandboxedIframe`、JS REPL、attachments/file-download provider 或完整 artifact tool renderer parity。
...
- Artifact/sandbox 仍是本地 baseline：还没有上游 `javascript_repl` tool、attachments runtime provider、file-return download provider、artifact logs、full artifact tool update/rewrite semantics、specialized Docx/Excel/Pdf/Svg/Markdown/Text viewers、reusable component package export 或 release/static browser smoke
