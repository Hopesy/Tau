...
- `src/Tau.Pods/Cli/PodsCli.cs`: follow-up fix disposes the process-level `JsonlTauLogSink` when an in-process CLI invocation completes. This keeps Windows tests from holding `.tau/log.jsonl` open after `PodsCli.RunAsync(...)` returns.
...
- `dotnet test tests/Tau.Pods.Tests/Tau.Pods.Tests.csproj --no-restore --verbosity minimal` — 32/32 after the Windows log-sink handle fix.
...
- **Sink lifetime is explicit for in-process CLI seams**: process exit still reclaims CodingAgent's handle, but `PodsCli.RunAsync(...)` is invoked inside unit tests, so it must dispose the sink before returning. Without that, Windows keeps `.tau/log.jsonl` open and recursive temp-directory cleanup can fail.
