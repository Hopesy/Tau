...
- 本轮开始时 committed baseline 已到 `1cd2ec8 feat(pods): add no-arg stop all command`，`main` 与 `origin/main` 对齐；该提交已包含 Pods no-arg stop-all 本地合同切片、history 和部分文档同步。
...
| Tau.Pods | 可用的 `agent` / prompt execution、real SSH/SCP/HF/setup/GPU/vLLM smoke、upstream local `model_run.sh` / realtime log-tail / startup streaming exact flow、multi-version rollback/transport hardening；`pods/start/list/stop/logs/ssh/shell` top-level 本地 shim、no-arg stop-all、`agent` prompt 参数映射 baseline、usage-aware GPU allocation state、deploy PID state writeback、duplicate configured deployment guard、upstream `~/.vllm_logs/<name>.log` path baseline 和 startup marker health/status baseline 已有合同测试但仍不是真实 remote e2e。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
...
- Upstream operation compatibility 仍未最终关闭：`pods` group、top-level `start/list/stop`、no-arg stop-all、`shell` 和既有 `ssh/logs` 已有本地 shim/contract tests，`agent` 也已有 prompt 参数映射 baseline；但可用 pod-agent execution、direct PID/log-tail/startup streaming flow 和真实 prompt command runtime 仍需实现或非目标确认。
...
- GPU/model allocation 当前关闭的是本地 baseline：已完成 `--gpus` / `--memory` / `--context` planning、基于 `pod.models[*].gpu` usage 的 least-used GPU 选择、成功 deploy/stop 后的 configured model state 写回/删除、deploy stdout `pid=<n>` 的 best-effort PID 写回、upstream `~/.vllm_logs/<name>.log` log path baseline，以及 health/status 对 startup complete/failure marker 的本地判定。仍缺 exact `model_run.sh` wrapper、实时 startup watcher/log-tail streaming、多版本 rollout/rollback、long-running remote transport hardening 和 real ops smoke。
- Start/deploy 本地防护当前又补上 duplicate configured deployment guard：`vllm deploy` 和 top-level `start` 在远端执行前拒绝同名 configured model，输出 `failureKind=model-already-exists` 并保留原 state。它只关闭上游 `pod.models[name]` 已存在时拒绝启动的本地合同，不等于上游 `model_run.sh` watcher 失败后删除配置的完整流程。
