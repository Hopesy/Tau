...
- 最新 CodingAgent package manager / extension runtime baseline 已关闭一层本地配置、资源、安装执行和 JavaScript/TypeScript command 执行合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts` / `core/extensions/loader.ts` / `core/extensions/runner.ts` / `core/agent-session.ts` / `utils/git.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest、约定目录和 package object filters，并支持 plain include、glob include、`!` exclude、`+` exact force-include、`-` exact force-exclude 资源过滤，把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；extension resource discovery 现在按上游式 `.ts/.js` direct file、目录 `index.ts/js` 和 `package.json pi.extensions` entry 解析，同时保留 Tau JSON command file 兼容；`.js` 与 `.ts` extension module 现在会通过本地 Node 子进程的 limited runtime 加载，`.ts` 路径使用 Node 24 type-stripping hooks，支持 relative `.ts` import 和 package-loaded `node_modules` 下 erasable TypeScript entry，支持 `pi.registerCommand(...)` 注册 slash command，`/extensions` 会显示 `(project, javascript|typescript)` 命令和 `loaded; commands N; limited runtime` module status，`TryInvoke` 会执行 JS/TS handler，并支持 handler 返回 status text 或通过 `pi.sendMessage(...)` / `ctx.sendMessage(...)` 发送 runner 输入。npm package install/remove/update 仍通过可注入 command runner 执行全局 `npm install -g` / project `.tau/npm --prefix` / configured `npmCommand`，git source 支持 `git:`、HTTPS、SSH 等常见形式的 clone/checkout/pull/remove 到 user/project `.tau/git` install root，`PI_OFFLINE` 会跳过 update，命令失败不会写入 settings，错误输出走现有 secret redaction。`/reload` 会重新解析 package resources 后再刷新 extensions。该 package/extension 切片仍不等于完整 `@mariozechner/jiti` import/alias/virtualModules parity，不注册 extension custom tools/flags/shortcuts/events，不提供真实 extension UI 调用或 interactive config selector，不关闭最终 `pi` package/bin identity、真实 npm/git registry/network smoke 或真实 package consumer smoke。
...
- RPC/extension/custom tool runtime 未关闭：JavaScript/TypeScript `registerCommand` limited runtime baseline 已有，但完整 `@mariozechner/jiti` import/alias/virtualModules parity、`registerTool`、extension UI real calls、actual hooks/custom tools migration to extensions、package/extension shortcut migration、flags/shortcuts/events lifecycle 和 general custom tool runtime parity 仍缺。
