...
| Tau.CodingAgent | top-level package source/list/config/local resource/filter baseline、npm/git package install/update execution baseline、startup changelog/install telemetry runtime baseline 与 assistant usage cost persistence baseline 已有；仍缺 top-level CLI flags、full TreeSelector/session schema、RPC exact schema / TypeScript extension runtime、custom tool/extension runtime、image/clipboard ingestion、full settings runtime wiring、share/export transcript parity 和真实 provider usage-cost 样本。 | CLI/RPC/schema targeted tests、package install/update smoke、extension UI headless contract tests、image/file/clipboard smoke、session tree migration/import/export tests、real provider vision/share e2e。 |
...
- 产品输入/输出缺口仍在：image/clipboard ingestion、provider vision e2e、full share/export/runtime transcript semantics、真实 provider usage-cost 样本 / UI 展示和 higher-level default path regression 仍未完成；本地 CodingAgent session 已默认持久化 assistant `usage.cost`。
