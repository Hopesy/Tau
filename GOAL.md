...
- 本轮开始时 committed baseline 已到 `008fce5 feat(pods): guard duplicate vllm deployments`，`main` 与 `origin/main` 对齐；该提交已包含 Pods duplicate configured deployment guard、operation `failureKind` 输出、history 和部分文档同步。
...
- 最新 Pods vLLM log path baseline 已关闭一层本地 remote log schema 合同：对照上游 `commands/models.ts` / `model_run.sh` 统一使用 `~/.vllm_logs/<name>.log` 的日志路径，Tau 的 `PodVllmCommandPlanner` 现在在 plan/metadata JSON 暴露 `logPath`，systemd unit 通过 `StandardOutput/StandardError=append:%h/.vllm_logs/<name>.log` 写入同一路径，fallback `nohup` 也写 `~/.vllm_logs/<name>.log`，top-level `logs` 快照会先 tail 该上游路径再回退旧 journal/`.tau_pods` 路径。该能力仍不关闭 exact `model_run.sh` wrapper、startup watcher、startup complete/failure line parsing、真实 SSH/HF/GPU/vLLM smoke 或多版本 rollback。
...
| Tau.Pods | 可用的 `agent` / prompt execution、real SSH/SCP/HF/setup/GPU/vLLM smoke、upstream local `model_run.sh` / startup watcher exact flow、multi-version rollback/transport hardening；`pods/start/list/stop/logs/logs --follow/ssh/shell` top-level 本地 shim、no-arg stop-all、`agent` prompt 参数映射 baseline、usage-aware GPU allocation state、deploy PID state writeback、duplicate configured deployment guard、upstream `~/.vllm_logs/<name>.log` path baseline、realtime follow baseline 和 startup marker health/status baseline 已有合同测试但仍不是真实 remote e2e。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
...
- Upstream operation compatibility 仍未最终关闭：`pods` group、top-level `start/list/stop`、no-arg stop-all、`shell`、既有 `ssh/logs` 和 `logs --follow` 已有本地 shim/contract tests，`agent` 也已有 prompt 参数映射 baseline；但可用 pod-agent execution、deploy-time startup watcher flow 和真实 prompt command runtime 仍需实现或非目标确认。
...
- GPU/model allocation 当前关闭的是本地 baseline：已完成 `--gpus` / `--memory` / `--context` planning、基于 `pod.models[*].gpu` usage 的 least-used GPU 选择、成功 deploy/stop 后的 configured model state 写回/删除、deploy stdout `pid=<n>` 的 best-effort PID 写回、upstream `~/.vllm_logs/<name>.log` log path baseline、`logs --follow` realtime tail baseline，以及 health/status 对 startup complete/failure marker 的本地判定。仍缺 exact `model_run.sh` wrapper、deploy-time startup watcher、多版本 rollout/rollback、long-running remote transport hardening 和 real ops smoke。
- Start/deploy 本地防护当前又补上 duplicate configured deployment guard：`vllm deploy` 和 top-level `start` 在远端执行前拒绝同名 configured model，输出 `failureKind=model-already-exists` 并保留原 state；`logs --follow` 也对齐上游 `viewLogs` 的 configured model guard，missing model 输出 `failureKind=model-not-found` 且不执行 SSH。它们只关闭对应本地合同，不等于上游 `model_run.sh` watcher 失败后删除配置的完整流程。
