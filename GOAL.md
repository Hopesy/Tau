...
- RPC/extension/custom tool runtime 未关闭：JavaScript/TypeScript `registerCommand` limited runtime baseline、`registerTool` schema/metadata/basic execute/prepareArguments baseline 和 `tool_call` block/input mutation / `tool_result` rewrite hook baseline 已有，但完整 `@mariozechner/jiti` import/alias/virtualModules parity、rich tool content/renderers/details 语义、extension UI real calls、actual hooks/custom tools migration to extensions、package/extension shortcut migration、CLI flag value injection、interactive shortcut dispatch、shortcut conflict diagnostics、其它 lifecycle events、运行中 runner tool hot-swap 和 general custom tool runtime parity 仍缺。
