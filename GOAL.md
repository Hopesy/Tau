...
- Config/schema 未关闭：`PI_CONFIG_DIR -> pods.json`、无 env `~/.pi/pods.json` 默认和 missing config 空对象语义已有 Tau baseline；upstream `pods: Record<string, Pod>` / `models: Record<string, Model>` schema migration、active pod compatibility 和 state migration 仍需最终审计。
...
1. 先关闭低歧义 contract 缺口：Pods record-shaped config/schema migration 与 active/state compatibility；CodingAgent RPC extension UI protocol；CodingAgent image/clipboard ingestion；WebUi sandbox/tool/artifact runtime API。
