...
- 本轮开始时 committed baseline 已到 `ac9bd86 feat(pods): add vllm runner wrapper`，`main` 本地领先 `origin/main` 1 个提交；该提交已包含 Pods runner/wrapper/pseudo-TTY baseline、deploy-time startup watcher、realtime log follow、duplicate configured deployment guard、operation `failureKind` 输出、history 和文档同步。
...
- 按当前 matrix 全表 status 列机器计数为 `partial=189`、`external-e2e-needed=31`、`ported=32`、`missing=9`、`non-goal-proposed=1`、`verified=0`。这说明当前仓库是高覆盖 baseline + 大量局部合同测试状态，不是 100% 移植完成状态；`ported` 也只是“已有本地证据但未达到最终验收”，不能当完成。
- 当前 `missing` 行集中在 package manager/install telemetry/changelog、RPC extension UI protocol、image/attachment ingestion、WebUi sandbox/tool/artifact runtime/prompt constants、Pods prompt/agent execution、root release/scripts parity。任何一个 `missing` 未关闭，100% 验收都不能通过。
...
- 最新 Pods top-level command shim 已关闭一层本地合同：`pods` / `pods setup` / `pods active` / `pods remove` 复用配置与注册路径，`start <model> --name <name>` 复用 `vllm deploy` 并强制上游 `--name` 要求，`list` 复用 active/指定 pod deployments，`stop <name>` 复用 `vllm stop`；no-arg `stop [--config path] [--pod id]` 会先列出 active/指定 pod deployments，再逐个复用 `vllm stop` 停止，JSON 输出聚合 stop-all 结果。本轮继续补 `shell [--config path] [--pod id] [pod]`，通过无远端命令 SSH 进程打开 active/指定 pod，并补 `agent [--config path] [--pod id] <name> [message/options...]` 的上游 prompt 参数映射 baseline：读取 pod configured model，构造 `--base-url` / `--model` / redacted `--api-key` / `--api` / `--system-prompt` 和 passthrough user args，但保持执行未实现的失败合同。该命令合同已通过 PodsCli targeted 和完整 Pods tests。它仍不关闭上游本地 PID/log-tail/startup streaming exact flow、可用的 pod-agent runtime、真实 SSH/HF/GPU/vLLM smoke 或 Pods final `verified` 状态。
- 最新 Pods GPU allocation state baseline 已关闭本地 least-used GPU 选择与配置状态维护合同：对照上游 `packages/pods/src/commands/models.ts`，`PodVllmCommandPlanner` 现在从 configured `pod.Models[*].Gpu` 统计 usage 并选择最少使用 GPU；成功 `vllm deploy` / top-level `start` 写回 configured model 的 model/port/gpu state，成功 `vllm stop` / top-level `stop` / no-arg stop-all 删除对应 state。该能力仍不关闭上游真实 PID 捕获、`~/.vllm_logs` startup streaming、可用 pod-agent runtime、真实 SSH/HF/GPU/vLLM smoke、多版本 rollback 或 Pods final `verified` 状态。
...
| --- | --- | --- |
| Inventory coverage | Phase 1 grouped inventory 已覆盖 7 个上游 package 和 root scripts；按当前 matrix 全表 status 列统计仍是 `partial=189`、`external-e2e-needed=31`、`ported=32`、`missing=9`、`non-goal-proposed=1`、`verified=0`。 | 每个 file/surface/root-script row 必须最终转成 `verified`，或有用户明确确认的 `non-goal`；不得保留 `partial`、`missing`、`ported`、`external-e2e-needed`。 |
...
- Upstream operation compatibility 仍未最终关闭：`pods` group、top-level `start/list/stop`、no-arg stop-all、`shell`、既有 `ssh/logs` 和 `logs --follow` 已有本地 shim/contract tests，`agent` 也已有 prompt 参数映射 baseline；deploy-time startup watcher 已有本地结构化 baseline，但可用 pod-agent execution、真实 prompt command runtime 和真实 remote e2e 仍需实现或非目标确认。
...
- GPU/model allocation 当前关闭的是本地 baseline：已完成 `--gpus` / `--memory` / `--context` planning、基于 `pod.models[*].gpu` usage 的 least-used GPU 选择、成功 deploy/stop 后的 configured model state 写回/删除、deploy stdout `pid=<n>` 的 best-effort PID 写回、upstream `~/.vllm_logs/<name>.log` log path baseline、`logs --follow` realtime tail baseline，health/status 对 startup complete/failure marker 的本地判定，deploy-time startup watcher ready/failure/rollback/no-writeback baseline，本地 `model_run_<name>.sh` / `model_wrapper_<name>.sh` / pseudo-TTY `script` 启动 baseline，以及 runner HF download / PI API key command baseline。仍缺多版本 rollout/rollback、long-running remote transport hardening 和 real ops smoke。
- Start/deploy 本地防护当前又补上 duplicate configured deployment guard：`vllm deploy` 和 top-level `start` 在远端执行前拒绝同名 configured model，输出 `failureKind=model-already-exists` 并保留原 state；`logs --follow` 也对齐上游 `viewLogs` 的 configured model guard，missing model 输出 `failureKind=model-not-found` 且不执行 SSH；deploy-time startup watcher 失败时会 rollback 且不写入 configured model state。它们只关闭对应本地合同，不等于 exact 上游 `model_run.sh` save-before-watch/delete-config 流程或真实 vLLM runtime 证据。
...
- 每轮开始重新机器计数 matrix 状态，当前全表校准值为 `partial=189`、`external-e2e-needed=31`、`ported=32`、`missing=9`、`non-goal-proposed=1`、`verified=0`；如果 matrix、GOAL、next、quality 对同一缺口有矛盾，先修文档事实再实现。
...
本轮审视后，下一步不是重新做 inventory，而是从当前 `missing=9`、`external-e2e-needed=31` 和高阻塞 `partial` 行中领取可验证切片：
