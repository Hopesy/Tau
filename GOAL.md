...
- 最新 CodingAgent package manager / extension entry discovery baseline 已关闭一层本地配置、资源和安装执行合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts` / `core/extensions/loader.ts` / `utils/git.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest、约定目录和 package object filters，并支持 plain include、glob include、`!` exclude、`+` exact force-include、`-` exact force-exclude 资源过滤，把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；extension resource discovery 现在按上游式 `.ts/.js` direct file、目录 `index.ts/js` 和 `package.json pi.extensions` entry 解析，同时保留 Tau JSON command file 兼容；`/extensions` 会报告发现的 TypeScript/JavaScript module entry，并明确标记为 `runtime pending`。npm package install/remove/update 仍通过可注入 command runner 执行全局 `npm install -g` / project `.tau/npm --prefix` / configured `npmCommand`，git source 支持 `git:`、HTTPS、SSH 等常见形式的 clone/checkout/pull/remove 到 user/project `.tau/git` install root，`PI_OFFLINE` 会跳过 update，命令失败不会写入 settings，错误输出走现有 secret redaction。`/reload` 会重新解析 package resources 后再刷新 extensions。该 package/extension discovery 切片仍不执行 TypeScript extension runtime、不注册 extension custom tools/commands/flags，不提供 interactive config selector，不关闭最终 `pi` package/bin identity、真实 npm/git registry/network smoke 或真实 package consumer smoke。
