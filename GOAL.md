...
- 最新 CodingAgent package manager baseline 已关闭一层本地配置、资源和安装执行合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts` / `utils/git.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest、约定目录和 package object filters，并支持 plain include、glob include、`!` exclude、`+` exact force-include、`-` exact force-exclude 资源过滤，把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；npm package install/remove/update 现在通过可注入 command runner 执行全局 `npm install -g` / project `.tau/npm --prefix` / configured `npmCommand`，git source 支持 `git:`、HTTPS、SSH 等常见形式的 clone/checkout/pull/remove 到 user/project `.tau/git` install root，`PI_OFFLINE` 会跳过 update，命令失败不会写入 settings，错误输出走现有 secret redaction。`/reload` 会重新解析 package resources 后再刷新 extensions。该 package manager 切片仍不提供 interactive config selector，不关闭 package-loaded TypeScript extension runtime、最终 `pi` package/bin identity、真实 npm/git registry/network smoke 或真实 package consumer smoke。
...
- 最新 CodingAgent RPC assistant stream event schema baseline 已关闭一层本地合同：对照上游 `rpc-types.ts` / `rpc-mode.ts` / `docs/rpc.md` / `packages/ai/src/types.ts`，Tau RPC `message_update` 现在输出上游 `assistantMessageEvent` 字段，不再输出 Tau 私有 `streamEvent`；assistant stream event payload 补齐 `partial`、`text_end/thinking_end.content`、`toolcall_end.toolCall` 和 `done/error.reason` baseline。该能力只关闭本地 `message_update` assistant event shape；完整 TypeScript extension runtime、package-loaded extensions、custom tools、全量 response/message schema、toolCall arguments object shape、settings/package/terminal contracts 和最终 RPC `verified` 状态仍保持 open。
