...
- 产品输入/输出缺口仍在：image/clipboard ingestion、provider vision e2e、full share/export/runtime transcript semantics、真实 provider usage-cost 样本、richer footer/TUI 展示和 higher-level default path regression 仍未完成；本地 CodingAgent session 已默认持久化 assistant `usage.cost`，`/session` 与 RPC `get_session_stats` 已展示本地 persisted usage/cost totals。
