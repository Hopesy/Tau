...
- 当前 `missing` 行已被继续收窄：CodingAgent top-level package source/list/config/local package resource baseline 与 npm/git install/update/remove 执行 baseline 已落地，但 startup changelog/version state、install telemetry runtime、WebUi 剩余 artifact/message reconstruction/runtime surfaces 和 root release/scripts parity 仍未关闭。任何一个 `missing` 未关闭，100% 验收都不能通过。
...
- 最新 CodingAgent package manager baseline 已关闭一层本地配置、资源和安装执行合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts` / `utils/git.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest 或约定目录，并把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；npm package install/remove/update 现在通过可注入 command runner 执行全局 `npm install -g` / project `.tau/npm --prefix` / configured `npmCommand`，git source 支持 `git:`、HTTPS、SSH 等常见形式的 clone/checkout/pull/remove 到 user/project `.tau/git` install root，`PI_OFFLINE` 会跳过 update，命令失败不会写入 settings，错误输出走现有 secret redaction。`/reload` 会重新解析 package resources 后再刷新 extensions。该切片仍不解析 glob/negation filter、不提供 interactive config selector，不关闭 package-loaded TypeScript extension runtime、startup changelog/version telemetry、最终 `pi` package/bin identity、真实 npm/git registry/network smoke 或真实 package consumer smoke。
...
| Tau.CodingAgent | top-level package source/list/config/local resource baseline 已有；仍缺 npm/git package install/update execution、install telemetry/startup changelog runtime、top-level CLI flags、full TreeSelector/session schema、RPC exact schema / TypeScript extension runtime、custom tool/extension runtime、image/clipboard ingestion、full settings runtime wiring、usage cost/share/export transcript parity。 | CLI/RPC/schema targeted tests、package install/update smoke、extension UI headless contract tests、image/file/clipboard smoke、session tree migration/import/export tests、real provider vision/share e2e。 |
...
1. 先关闭低歧义 contract 缺口：Pods top-level command compatibility / operation schema；CodingAgent RPC exact schema / TypeScript extension runtime；CodingAgent image/clipboard ingestion；WebUi sandbox/tool/artifact runtime API。
