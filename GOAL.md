...
- [x] Latest committed implementation checkpoint：截至本次 `/goal` 计划刷新前，HEAD 为 `f99d1ba feat(coding-agent): align session startup flags`，`main...origin/main [ahead 47]`，工作树干净；上一切片已把 CodingAgent 启动期 `--session`、`--continue`、`--session-dir`、`--fork`、`--resume`、`--models`、`--list-models`、`--verbose` 本地 baseline 合并进当前事实面。
- [x] Latest verified local gate：上一实现切片通过 focused CodingAgent gate 77/77、完整 `Tau.CodingAgent.Tests` 615/615、完整 `Tau.WebUi.Tests` 61/61、`--list-models gemini` smoke、`--resume --print` 非交互诊断 smoke 和 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-dotnet.ps1 -SkipRestore`（Ai 287、Agent 123、Tui 251、CodingAgent 615、WebUi 61、Pods 216）。`-RunSmoke` 未在该切片复跑，仍属于最终验收链。
- [ ] Phase 2 critical contract closure：当前 matrix 机器计数为 `partial=197`、`external-e2e-needed=31`、`ported=32`、`missing=1`、`non-goal-proposed=1`、`verified=0`。下一轮从 `Phase 2 Candidate Queue` 领取 contract/runtime/e2e/release 中的一个互斥切片，不重开 inventory。
...
- [ ] Phase 5 release/package/CI final parity：当前唯一 `missing` 行是 `Root release/scripts parity`；Tau release/CI 必须产出真实 executable/package artifacts，并完成 registry/signing/provenance/non-host smoke 或取得用户明确非目标确认。
...
本节是 2026-06-12 基于当前 HEAD、上游目录、active matrix 和本地工作树重新审视后的事实面，用来防止把 baseline、fake tests 或局部 planning 误写成 100% 完成。每次 `/goal` 续跑前都要重新确认这些值；如果当前仓库已经变化，以现场 `git status`、matrix 计数和验证结果为准。

- 本轮计划刷新时 committed baseline 为 `f99d1ba feat(coding-agent): align session startup flags`，`main` 本地领先 `origin/main` 47 个提交，工作树干净。该提交已包含 CodingAgent startup/session/model flag baseline、WebUi Windows store/fixture 稳定性修复、`verify-dotnet.ps1` 串行化和默认完整 gate 证据。
...
- 按当前 matrix 全表 status 列机器计数为 `partial=197`、`external-e2e-needed=31`、`ported=32`、`missing=1`、`non-goal-proposed=1`、`verified=0`。这说明当前仓库是高覆盖 baseline + 大量局部合同测试状态，不是 100% 移植完成状态；`ported` 也只是“已有本地证据但未达到最终验收”，不能当完成。
- 当前唯一 `missing` 行是 `Root release/scripts parity`：root `build-binaries.sh` / release / package scripts 的最终 install/release chain 仍未证明到 100% 完整。任何一个 `missing` 未关闭，100% 验收都不能通过。
...
- 最新 CodingAgent package manager / extension runtime baseline 已关闭一层本地配置、资源、安装执行和 JavaScript/TypeScript command/tool 执行合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts` / `core/extensions/loader.ts` / `core/extensions/runner.ts` / `core/agent-session.ts` / `core/extensions/wrapper.ts` / `core/tools/tool-definition-wrapper.ts` / `utils/git.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest、约定目录和 package object filters，并支持 plain include、glob include、`!` exclude、`+` exact force-include、`-` exact force-exclude 资源过滤，把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；extension resource discovery 现在按上游式 `.ts/.js` direct file、目录 `index.ts/js` 和 `package.json pi.extensions` entry 解析，同时保留 Tau JSON command file 兼容；`.js` 与 `.ts` extension module 现在会通过本地 Node 子进程的 limited runtime 加载，`.ts` 路径使用 Node 24 type-stripping hooks，支持 relative `.ts` import 和 package-loaded `node_modules` 下 erasable TypeScript entry，支持 `pi.registerCommand(...)` 注册 slash command，`/extensions` 会显示 `(project, javascript|typescript)` 命令和 `loaded; commands N; tools N; limited runtime` module status，`TryInvoke` 会执行 JS/TS handler，并支持 handler 返回 status text 或通过 `pi.sendMessage(...)` / `ctx.sendMessage(...)` 发送 runner 输入；同一 limited runtime 现在也支持 `pi.registerTool(...)` 发现 JS/TS extension tool schema/metadata，并在 runner 启动时把 extension tools 合并进 Agent 工具集，同名 extension tool 按上游 extension runner 首个注册胜出、与内置工具同名时由 extension tool 覆盖，`prepareArguments` 会在 Agent schema validation 前调用 extension shim 并返回准备后的参数 object，tool execution 会回到 Node runtime 调用 `execute(toolCallId, params, signal, onUpdate, ctx)` 并返回文本型 `ToolResult` baseline；同一 runtime 现在还支持 `pi.on("tool_call")` / `pi.on("tool_result")` 的本地 hook baseline，runner 通过 `IToolInterceptor` 接入，允许 extension 阻断工具调用、按 handler/module 顺序改写 `event.input` 并让 mutation 后参数传给后续 handler 与实际工具执行，或改写文本 content / JSON details / isError；同一 runtime 还支持 `pi.registerFlag(...)` metadata/default 注册、`pi.getFlag(...)` 默认值读取和 `pi.registerShortcut(...)` metadata/status 注册，并在 `CodingAgentExtensionStatus`、`/extensions` 与非零 module status 中报告 flags/shortcuts；CLI 现在还会收集未知 `--flag[=value]` token，按上游 `applyExtensionFlagValues` 对照已注册 flag 校验（boolean -> true、string 需要值、未知 flag 与缺值 string flag 产出 error 诊断且返回非零退出码），把解析后的值注入 runtime 让 `pi.getFlag(...)` 返回 CLI 值并覆盖 `registerFlag` 默认值；同一 limited runtime 现在也支持 `pi.on("agent_start"|"agent_end"|"turn_start"|"turn_end"|"message_start"|"message_update"|"message_end"|"tool_execution_start"|"tool_execution_update"|"tool_execution_end")` 的只读生命周期广播 baseline，production runner 在产出对应 `AgentEvent` 前向 JS/TS handler 发送上游形状 payload，handler error 会以 `extension/event.error` 记录且不失败 run；同一 limited runtime 现在还固定了 virtual package import stub 子集，`@sinclair/typebox`、`@mariozechner/pi-ai`、`@mariozechner/pi-ai/oauth`、`@mariozechner/pi-agent-core`、`@mariozechner/pi-tui` 和 `@mariozechner/pi-coding-agent` 可参与真实 JS extension command/tool 注册与执行。npm package install/remove/update 仍通过可注入 command runner 执行全局 `npm install -g` / project `.tau/npm --prefix` / configured `npmCommand`，git source 支持 `git:`、HTTPS、SSH 等常见形式的 clone/checkout/pull/remove 到 user/project `.tau/git` install root，`PI_OFFLINE` 会跳过 update，命令失败不会写入 settings，错误输出走现有 secret redaction。`/reload` 会重新解析 package resources 后再刷新 extensions 和报告 tools/flags/shortcuts（有内容时），但运行中 runner tool registry 热替换仍未实现。该 package/extension 切片仍不等于完整 `@mariozechner/jiti` import/alias/virtualModules parity，renderer/details-rich content、session/provider/context/input/user_bash/model/resource/before hooks、等待型 extension UI dialog、custom UI、interactive widget factory、interactive config selector、最终 `pi` package/bin identity、真实 npm/git registry/network smoke 或真实 package consumer smoke 仍保持 open。
- 最新 CodingAgent CLI top-level flag baseline 已继续收窄：`--session <path>` / `--session=<path>` 已关闭 explicit flat/jsonl 文件路径分支；`--continue` / `-c` 和 `--session-dir <dir>` / `--session-dir=<dir>` 已关闭本地 JSONL tree session startup 分支，其中 `--session-dir` 会在指定目录创建新的 per-run JSONL session，组合 `--continue` 时选择该目录中最近有效 JSONL session，目录为空则新建；`--fork <path>` / `--fork=<path>` 已关闭直接 JSONL tree session 文件路径 fork baseline：启动时校验缺值和与 `--session` / `--continue` / `--resume` 的冲突，把源 session 当前 branch 导出到新 JSONL，并把目标放在显式 `--session-dir` 或默认 `.tau/coding-agent-sessions` 下，同时记录 `parentSession`；`--session <id-prefix>` 和 `--fork <id-prefix>` 现在会把无扩展、非路径参数按 JSONL session id 前缀解析，先查显式 `--session-dir`，再查当前可发现的默认/clone session 列表，匹配后分别打开 tree session 或 fork 当前 branch；`--resume` / `-r` 现在在交互式终端中打开本地 resume selector，选择当前可发现 JSONL session 后进入同一 tree session startup path，print/RPC 或无 selector 场景会明确报错；`--models <patterns>` 现在会按逗号解析当前进程临时 model scope，支持 exact `provider/model`、唯一 model id、`:thinking` 后缀和 `*` / `?` 通配展开，优先于 settings `enabledModels` 参与启动模型选择与 Ctrl+P/RPC model cycling，但不写回 settings；`--list-models [search]` 现在在打开 session 前列出模型表并退出，包含 provider/model/context/max-out/thinking/images 和本地 fuzzy search；`--verbose` 现在会覆盖 `quietStartup` 并在交互启动时显示 model scope notice。当前能力仍只关闭本地启动期 session/model flag baseline；session id / partial id 全局 `listAll` lookup、cross-project fork prompt、完整 upstream SessionManager schema/selection、完整 fuzzy/glob model scope parity、真实 auth/provider e2e 和 final package/bin identity 仍保持 open。
- 最新 CodingAgent JS/TS extension UI fire-and-forget RPC bridge 已关闭一层本地合同：对照上游 `core/extensions/types.ts`、`modes/rpc/rpc-types.ts` 和 `modes/rpc/rpc-mode.ts`，Tau 在 RPC host 绑定 extension store 后会向真实 JS/TS extension handler 暴露 `ctx.hasUI=true`，并把 handler 内 `ctx.ui.notify(...)`、`setStatus(...)`、string-array `setWidget(...)`、`setTitle(...)`、`setEditorText(...)` 和 `pasteToEditor(...)` 收集为上游形状 `extension_ui_request`。同轮 focused 回归还固定了当前 limited runtime 的 virtual package import stub 子集，覆盖 `@sinclair/typebox` 与 `@mariozechner/pi-ai` / `pi-ai/oauth` / `pi-agent-core` / `pi-tui` / `pi-coding-agent` 的本地 command/tool 注册执行路径。当前只关闭 fire-and-forget UI request 与 limited virtual import stub baseline；等待型 `select/confirm/input/editor`、custom UI、interactive TUI widget factory、完整 jiti import/alias/virtualModules、session/provider/context/input/user_bash/model/resource/before hooks、运行中 runner tool hot-swap、package consumer/network smoke 和最终 extension runtime `verified` 状态仍保持 open。
...
| --- | --- | --- |
| Inventory coverage | Phase 1 grouped inventory 已覆盖 7 个上游 package 和 root scripts；按当前 matrix 全表 status 列统计仍是 `partial=197`、`external-e2e-needed=31`、`ported=32`、`missing=1`、`non-goal-proposed=1`、`verified=0`。 | 每个 file/surface/root-script row 必须最终转成 `verified`，或有用户明确确认的 `non-goal`；不得保留 `partial`、`missing`、`ported`、`external-e2e-needed`。 |
...
| Tau.CodingAgent | top-level package source/list/config/local resource/filter baseline、npm/git package install/update execution baseline、startup changelog/install telemetry runtime baseline、JavaScript/TypeScript extension `registerCommand` limited runtime baseline、JavaScript/TypeScript extension `registerTool` prepareArguments baseline、JavaScript/TypeScript extension `tool_call` block/input mutation / `tool_result` rewrite hook baseline、agent/message/turn/tool_execution lifecycle event broadcast non-blocking baseline、JS/TS extension fire-and-forget UI request bridge baseline、limited virtual package import stub baseline、assistant usage cost persistence baseline、本地 `/session` / RPC stats usage-cost display baseline、CLI/read_file PNG resize + EXIF dimension metadata baseline，以及本地启动期 `--session` / `--continue` / `--session-dir` / `--fork` / `--resume` / `--models` / `--list-models` / `--verbose` flag baseline 已有；仍缺 session id / partial id 全局 `listAll` lookup、cross-project fork prompt、完整 upstream SessionManager schema/selection、完整 fuzzy model scope parity、RPC exact schema / waiting extension UI dialogs / full jiti/import/alias/virtualModules/custom tool extension runtime、session/provider/context/input/user_bash/model/resource/before hooks、desktop/OSC52 image clipboard、full Photon image convert/resize parity、full settings runtime wiring、share/export transcript parity、真实 provider vision/usage-cost 样本和 richer footer/TUI 展示。 | CLI/RPC/schema targeted tests、package install/update smoke、extension UI headless contract tests、image/file/clipboard smoke、session tree migration/import/export tests、real provider vision/share e2e。 |
...
- `scripts/build-binaries.sh` / `scripts/release.mjs` 仍需关闭非宿主平台 executable smoke、真实 package registry、signing/provenance rehearsal、release archive signing、Photon/interactive raster assets/external export-html payload parity。
...
- RPC/extension/custom tool runtime 未关闭：JavaScript/TypeScript `registerCommand` limited runtime baseline、`registerTool` schema/metadata/basic execute/prepareArguments baseline、`tool_call` block/input mutation / `tool_result` rewrite hook baseline、CLI flag value injection baseline、interactive shortcut dispatch/conflict diagnostics baseline、agent/message/turn/tool_execution lifecycle event broadcast non-blocking baseline，以及真实 JS/TS handler 到 RPC `extension_ui_request` 的 fire-and-forget UI bridge 已有（handler error 记录为 `extension/event.error` 且不失败 run），但完整 `@mariozechner/jiti` import/alias/virtualModules parity、rich tool content/renderers/details 语义、等待型 extension UI dialogs、custom UI、interactive widget factory、actual hooks/custom tools migration to extensions、package/extension shortcut migration、session/provider/context/input/user_bash/model/resource/before hooks、运行中 runner tool hot-swap 和 general custom tool runtime parity 仍缺。
...
- 每轮开始重新机器计数 matrix 状态，当前全表校准值为 `partial=197`、`external-e2e-needed=31`、`ported=32`、`missing=1`、`non-goal-proposed=1`、`verified=0`；如果 matrix、GOAL、next、quality 对同一缺口有矛盾，先修文档事实再实现。
- 清理 matrix 中与最近实现不一致的 stale 文字，尤其是 CodingAgent startup/session/model flag baseline 已完成但全局 `listAll` lookup、cross-project fork prompt、完整 SessionManager schema/selection、真实 auth/provider e2e 和 final package/bin identity 仍 open 的状态表达。
- 将 backlog 按 `contract`、`runtime`、`external-e2e`、`release-package`、`final-audit` 五类归口。每个切片只能主攻一个类别；同一提交不得把无关模块、无关风险等级或 docs-only 状态刷新和 runtime 行为实现混成不可审查边界。
- 验收：`git diff --check` 通过，matrix/GOAL/next/quality 对同一缺口没有互相矛盾的完成声明；如果本轮只做文档校准，不需要跑全仓 `dotnet test`，但必须说明这是 docs-only gate。
...
- 验收：各模块 targeted tests 通过，public API compile samples 覆盖新增/变更合同，CLI/RPC/HTTP/jsonl output shape 有断言，matrix contract rows 只剩外部 e2e 或明确非目标。
...
- 验收：runtime feature 有 targeted tests、最小 smoke 和日志/输出证据；fake-only runner 只能证明合同，不能把真实产品行为标成完成。
...
- 验收：External E2E list 清零，或每个无法执行项都有用户确认的 `non-goal` 和对应 matrix 记录；所有日志、token、URL、账号和远端路径必须脱敏。
...
- 验收：release artifact 在干净目录可运行核心命令；package consumer smoke 能消费 `Tau.Ai` / `Tau.Agent` / `Tau.Tui`；registry/signing/provenance 不停留在 dry-run；唯一 `missing` 行 `Root release/scripts parity` 必须转成 `partial`、`verified` 或用户确认的 `non-goal`，不能长期保持 `missing`。
...
## Per-round acceptance checklist

每个 `/goal` 切片完成前必须满足以下检查。没有满足就继续修，不把半成品写成 checkpoint。

1. Scope：切片来自 matrix `Phase 2 Candidate Queue`、active plan 或本文件 `Next execution priority`，且只主攻一个 blocker class。
2. Evidence：已读取相关上游源码、Tau 当前 source/tests、active matrix row 和必要文档；涉及可能变化的外部 API 时已查官方资料、真实日志或真实测试。
3. Implementation：代码、测试、配置和脚本只改当前切片所需文件；不回滚用户未授权变更。
4. Validation：至少运行 targeted gate；触碰共享 runtime、public API、CLI/RPC/HTTP contract、持久化、auth/secret、release/package 时追加模块完整测试或项目级 `verify-dotnet.ps1 -SkipRestore`。
5. Documentation：同步 `GOAL.md`、active plan、matrix、`next.md`、`docs/QUALITY_SCORE.md` 中受影响的状态；只同步与当前行为变化相关的事实，不做大段无关重写。
6. History：仓库内容有实质修改时写 `docs/histories/YYYY-MM/YYYYMMDD-HHmm-<slug>.md`，说明用户诉求、改动、设计意图、文件和验证。
7. Commit boundary：形成可评审的单一提交边界；如果用户未要求提交，可以保持 clean diff checkpoint，但最终报告必须列出未提交文件和验证状态。

## /goal slice selection algorithm

后续收到 `继续` / `下一步` / `/goal` 时按这个顺序自动选择下一刀：

1. 如果工作树有未提交变更，先用 `git status --short --branch` 和 diff 判定它是否属于当前切片；属于则继续完成，不属于则隔离并避免覆盖。
2. 如果 matrix 仍有 `missing`，优先领取最小可验证 `missing` 行；当前唯一 `missing` 是 `Root release/scripts parity`。
3. 如果没有适合本轮关闭的 `missing`，领取阻塞范围最大的 `contract` 行：当前优先是 CodingAgent session global lookup/cross-project prompt、AI/Agent public package/export boundary、WebUi branch/tree session API、release/package consumer boundary。
4. 如果 contract 行已经有足够 baseline，推进 `runtime` 行：Tui live terminal、WebUi artifact/session product path、Mom Docker/Slack-compatible runtime、Pods setup/deploy/log/allocation remote-hardening。
5. 如果本机或用户环境具备真实凭证/服务，优先把对应 `external-e2e-needed` 行转成真实 e2e 证据；没有环境时保持 open，不伪造。
6. 每 3-5 个实现切片后执行一次主控校准：重算 matrix 计数，清理 stale docs，确认 `next.md` 没有与 matrix 冲突。
...
本轮审视后，下一步不是重新做 inventory，而是从当前 `missing=1`、`external-e2e-needed=31` 和高阻塞 `partial` 行中领取可验证切片：

1. Release/package root parity：关闭唯一 `missing` 行 `Root release/scripts parity` 的最小可验证子切片。优先选择 package consumer smoke、global alias/install parity、release/static browser smoke、non-host executable smoke、registry/signing/provenance rehearsal 中最小且本地可验证的一项；如果需要真实远端凭证或注册表，则保持 `external-e2e-needed`，但不能让 row 继续停留在“no complete release scripts”的旧表述。
2. CodingAgent session manager parity：继续刚完成的 startup flags 线，优先补全局 `SessionManager.listAll()` lookup、cross-project fork prompt、完整 upstream SessionManager schema/selection 中的本地可测 baseline；不要一刀混入 full jiti/custom UI/package network smoke。
3. AI / Agent public package boundary：继续 `Tau.Ai` / `Tau.Agent` 底座，收口 package/global install alias、public export/subpath 形状、Agent broader option pass-through、package consumer smoke 和 real proxy/e2e 边界；真实 provider/OAuth e2e 未跑前不得标 final。
4. WebUi branch/tree and artifact runtime：继续把 WebChat DTO baseline 推向 CodingAgent JSONL branch/tree true persistence、artifact message reconstruction、release/static browser smoke；本地 ASP.NET 测试不替代 packaged/static smoke。
5. Runtime/e2e lanes：Tui live terminal host / real TTY smoke、Mom Slack/Docker、Pods SSH/HF/GPU/vLLM、AI provider/OAuth/AWS 都要继续准备真实 smoke；没有环境就保持 `external-e2e-needed`，只做可本地证明的 blocker reduction。
6. 每轮只领取一个可审计切片，完成后同步 matrix、next、quality、history 和必要 docs；没有真实 e2e 环境时保持 open，不做完成声明。
