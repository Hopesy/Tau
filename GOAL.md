...
| Tau.Pods | 可用的 `agent` / prompt execution、real SSH/SCP/HF/setup/GPU/vLLM smoke、upstream local PID/log-tail/startup streaming exact flow、multi-version rollback/transport hardening；`pods/start/list/stop/logs/ssh/shell` top-level 本地 shim、no-arg stop-all、`agent` prompt 参数映射 baseline 和 usage-aware GPU allocation state 已有合同测试但仍不是真实 remote e2e。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
...
- GPU/model allocation 当前关闭的是本地 baseline：已完成 `--gpus` / `--memory` / `--context` planning、基于 `pod.models[*].gpu` usage 的 least-used GPU 选择，以及成功 deploy/stop 后的 configured model state 写回/删除。仍缺 exact PID/log-tail/startup streaming、多版本 rollout/rollback、long-running remote transport hardening 和 real ops smoke。
...
- 清理 matrix 中与最近实现不一致的 stale 文字，尤其是 Pods 本地 `--gpus` / `--memory` / `--context` planning、`PI_CONFIG_DIR` / no-env `~/.pi/pods.json` / missing config 空对象语义、record-shaped config/schema、top-level command shim 与 usage-aware GPU allocation state 已完成本地 baseline，但真实 remote e2e、exact PID/log-tail/startup streaming 和 Pods final `verified` 仍 open 的状态表达。
