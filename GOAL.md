...
- 当前 `missing` 行已被继续收窄：CodingAgent top-level package source/list/config/local package resource baseline 已落地，但 npm/git 安装执行、startup changelog/version state、install telemetry runtime、WebUi 剩余 artifact/message reconstruction/runtime surfaces 和 root release/scripts parity 仍未关闭。任何一个 `missing` 未关闭，100% 验收都不能通过。
...
- 最新 CodingAgent package manager baseline 已关闭一层本地配置和资源合同：对照上游 `package-manager-cli.ts` / `core/package-manager.ts` / `core/settings-manager.ts`，Tau 顶层 `install/remove/uninstall/update/list/config` 会在 runner 初始化前处理 package command，`install/remove -l` 写 project settings，默认写 user settings；settings 现在可读写上游 string/object `packages` source 形状；本地 package source 会解析 `package.json` `pi.extensions/skills/prompts/themes` manifest 或约定目录，并把 package resources 与 extension-contributed resources 合并给 extension/prompt/skill/theme stores；`/reload` 会重新解析 package resources 后再刷新 extensions。该切片不执行 npm/git install/pull/update、不解析 glob filter、不提供 interactive config selector，不关闭 startup changelog/version telemetry 或 TypeScript extension runtime。
...
- CLI/session/config/package parity 未最终关闭：package source/list/config/local resource baseline 已有；完整 TreeSelector/session schema、package/bin identity、npm/git install/update execution、settings/runtime auth parity、credential refresh UX、migration helpers 到真实 runtime 的覆盖仍需收口。
