...
- 最新 Pods top-level command shim 已关闭一层本地合同：`pods` / `pods setup` / `pods active` / `pods remove` 复用配置与注册路径，`start <model> --name <name>` 复用 `vllm deploy` 并强制上游 `--name` 要求，`list` 复用 active/指定 pod deployments，`stop <name>` 复用 `vllm stop`；本轮继续补上游 `pi stop` 无参语义，`stop [--config path] [--pod id]` 会先列出 active/指定 pod deployments，再逐个复用 `vllm stop` 停止，JSON 输出聚合 stop-all 结果。该命令合同已通过 PodsCli targeted 和完整 Pods tests。它仍不关闭 `shell`、`agent` / prompt mapping、上游本地 PID/log-tail/startup streaming exact flow、真实 SSH/HF/GPU/vLLM smoke 或 Pods final `verified` 状态。
...
| Tau.Pods | `shell`、`agent` / prompt mapping、usage-aware GPU allocation state、real SSH/SCP/HF/setup/GPU/vLLM smoke、upstream local PID/log-tail/startup streaming exact flow、multi-version rollback/transport hardening；`pods/start/list/stop/logs/ssh` top-level 本地 shim 与 no-arg stop-all 已有合同测试但仍不是真实 remote e2e。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
...
- Upstream operation compatibility 仍未最终关闭：`pods` group、top-level `start/list/stop`、no-arg stop-all 和既有 `ssh/logs` 已有本地 shim/contract tests，但 `shell`、`agent` / prompt mapping、direct PID/log-tail/startup streaming flow 和 pod prompt command mapping 仍需实现或非目标确认。
