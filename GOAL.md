...
| Tau.Pods | 可用的 `agent` / prompt execution、real SSH/SCP/HF/setup/GPU/vLLM smoke、upstream local `model_run.sh` / realtime log-tail / startup streaming exact flow、multi-version rollback/transport hardening；`pods/start/list/stop/logs/ssh/shell` top-level 本地 shim、no-arg stop-all、`agent` prompt 参数映射 baseline、usage-aware GPU allocation state、deploy PID state writeback 和 upstream `~/.vllm_logs/<name>.log` path baseline 已有合同测试但仍不是真实 remote e2e。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
