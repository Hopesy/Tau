...
- 当前 committed baseline 到 `67fedeb feat(pods): add PI_CONFIG_DIR config support`，`main` 与 `origin/main` 对齐；工作树仍有既有 `.github/workflows/tau-ci.yml` 删除状态。该删除不属于本轮 100% plan 证据，不得在没有单独 CI 审计时顺手 stage 或用于完成声明。
...
- 当前 matrix 表格行机器计数为 `partial=110`、`external-e2e-needed=13`、`ported=12`、`missing=8`、`verified=0`。这说明当前仓库是高覆盖 baseline + 大量局部合同测试状态，不是 100% 移植完成状态；`ported` 也只是“已有本地证据但未达到最终验收”，不能当完成。
- 当前 `missing` 行集中在 package/install telemetry/changelog、RPC extension UI protocol、image/attachment ingestion、WebUi sandbox/tool/artifact runtime、Pods prompt/agent/pod-model user command、root release/scripts parity。任何一个 `missing` 未关闭，100% 验收都不能通过。
...
- 最新 Pods config path/env 切片只关闭 `PI_CONFIG_DIR -> pods.json` 子合同：对照上游 `packages/pods/src/config.ts`，Tau 已在 env 存在时默认读写 `<PI_CONFIG_DIR>/pods.json`，且显式 `--config path` / positional config path 继续优先；但无 env 时仍保留 Tau 默认 `tau.pods.json`，missing config 语义仍不同于上游 `{ pods: {} }`，所以该项仍是 `partial`。
...
| --- | --- | --- |
| Inventory coverage | Phase 1 grouped inventory 已覆盖 7 个上游 package 和 root scripts；当前 matrix 仍是 `partial=110`、`external-e2e-needed=13`、`ported=12`、`missing=8`、`verified=0`。 | 每个 file/surface/root-script row 必须最终转成 `verified`，或有用户明确确认的 `non-goal`；不得保留 `partial`、`missing`、`ported`、`external-e2e-needed`。 |
...
| External e2e | 当前大多是 fake/stub/contract tests；matrix 至少 13 行明确需要真实 provider/service/runtime e2e。 | 用真实凭证、真实服务、真实容器或真实远端 pod 跑脱敏 smoke；没有环境时保持 open，最终只能由用户确认 `non-goal` 关闭。 |
...
- 每轮开始重新机器计数 matrix 状态，当前校准值为 `partial=110`、`external-e2e-needed=13`、`ported=12`、`missing=8`、`verified=0`；如果 matrix、GOAL、next、quality 对同一缺口有矛盾，先修文档事实再实现。
- 清理 matrix 中与最近实现不一致的 stale 文字，尤其是 Pods 本地 `--gpus` / `--memory` / `--context` planning、`PI_CONFIG_DIR` env default 已完成但 final config/schema/e2e 仍 open 的状态表达。
...
本轮审视后，下一步不是重新做 inventory，而是从当前 `missing=8`、`external-e2e-needed=13` 和高阻塞 `partial` 行中领取可验证切片：

1. 先关闭低歧义 contract 缺口：Pods no-env 默认 `~/.pi/pods.json`、missing config `{ pods: {} }`、record-shaped config/schema migration；CodingAgent RPC extension UI protocol；CodingAgent image/clipboard ingestion；WebUi sandbox/tool/artifact runtime API。
