...
| Tau.Pods | top-level `pods/shell/ssh/start/stop/list/logs/agent` compatibility、prompt/agent mapping、usage-aware GPU allocation state、real SSH/SCP/HF/setup/GPU/vLLM smoke、startup log streaming、multi-version rollback/transport hardening。 | CLI alias/operation tests, real remote pod smoke, vLLM startup/health/log evidence, model allocation state tests. |
