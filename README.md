...
`verify-ai-provider-e2e-matrix.ps1` 是 AI provider/OAuth 的可审计矩阵脚本：默认 `inspect` 只读 provider/auth/model 状态，`-Isolated` 会把 `TAU_AUTH_FILE` 与 `TAU_MODELS_FILE` 指到临时空文件并清空常见 provider env，`-RunConfigured` 则只对已配置凭证的 provider 发起最小真实调用。默认 provider 列表覆盖 `openai`、`openai-codex`、`azure-openai-responses`、`github-copilot`、`anthropic`、`google`、`google-vertex`、`google-gemini-cli`、`google-antigravity`、`mistral` 和 `amazon-bedrock`。它证明的是本地 provider/OAuth 检查与可执行矩阵契约，不等于真实 cloud e2e 已完成。
