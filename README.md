...
- `Tau.Mom` 已从纯文本 worker 推进到 **支持结构化委派请求 + 本地 events + Slack-compatible message envelope + transport/responder seam + 附件 staging + workspace layout bootstrap + prompt debug snapshot** 的本地委派宿主
...
- 已有：
  ...
  - `Tau.Mom`：本地文件委派处理链，支持 `.txt/.md/.json` 请求、`provider/model/workingDirectory/title/metadata/attachments` 结构化字段、Slack-compatible channel message envelope、transport/responder/processor seam、本地附件 staging、本地 `events/*.json` 唤醒、workspace layout bootstrap、`last_prompt.jsonl` prompt debug snapshot、`--once`、outbox 结果落盘和 archive 归档
...
- `.txt/.md/.json` 请求和 `mom/events/*.json` 会先映射成 `MomChannelMessage` / `MomChannelAttachment`，统一 channel、user、ts、thread、attachment、provider/model/title/metadata 语义；当前这是给未来 Slack adapter/backfill/queue 复用的本地 envelope，不代表 Slack SDK 已接通
- `IMomChannelTransport` / `IMomChannelResponder` / `MomChannelMessageProcessor` 已固定未来 Slack adapter 的输入输出边界：transport 读入 channel message，responder 负责响应/thread/typing/upload，processor 负责 busy-state、stop placeholder、attachment staging、status/log writeback 与 runner 调用
- runner 执行前会确保本地 workspace/channel layout 存在：`scratch/`、workspace-level `skills/`、channel-level `skills/`、`attachments/` 和 `events/`
- runner 输入会先注入 `<mom_runtime_context>`，说明本地 workspace/channel layout、`SYSTEM.md`、`scratch/`、skill docs、events 文件格式、attachment manifest、memory/log/status 路径和 `[SILENT]` 事件响应约定；这只是本地 Mom 语义，不等于 Slack adapter 或 custom skill runtime 已接通
...
- 本地存在的 `attachments` 文件会复制到 `workingDirectory/attachments/<timestamp>_<filename>`，并通过 `attachments/attachments.jsonl` 记录 `original/local/source`；`log.jsonl` 写入时会保留上游 mom 风格的 `original/local` 附件元数据
...
- workspace/channel `skills/**/SKILL.md` 会被扫描为 prompt 中的 skill docs inventory，但当前只提供文档上下文，不代表这些 skill 已作为工具加载执行
...
- `workingDirectory/status.json` 会记录当前或最近一次 delegation 的 `running/completed/failed` 状态、请求文件、provider/model、workdir、时间、错误与响应摘要；本地 worker 会跳过同一 workdir 内未过期的 `running` 状态，默认 60 分钟后视为 stale，给后续 Slack busy-state / stop flow 留本地状态面
...
- `Tau.Agent.Tests` 已通过 20 个 Mom 结构化委派测试
