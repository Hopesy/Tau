...
`publish-release-packages.ps1` 是当前 package registry publish synchronization 的受控层：默认 dry-run，只预览 `dotnet pack` 与 `dotnet nuget push`；显式 `-Apply` 才会先打包再推送。默认项目边界是上游 workspace public library 对应的 `Tau.Ai`、`Tau.Agent`、`Tau.Tui`，应用项目仍默认通过 release archives 交付，只有显式 `-ProjectPath` 才会纳入并给出 application package warning。脚本从 `Directory.Build.props` 读取版本，默认输出到 `artifacts/nuget`，默认推送到 NuGet v3 source，并只从 `-ApiKeyEnv` 指定的环境变量读取 API key；JSON、命令预览和子进程输出预览只报告 env 名称与 present 状态，不回显密钥。真实发布必须显式 `-Apply`，并确认 package source、凭证和 library/application 包边界；`-SkipPush` 可只打包，`-PackagePath` / `-SkipPack` 可推送已存在包，`-SkipDuplicate` 对应 `dotnet nuget push --skip-duplicate`。

`verify-release-package-publish.ps1` 是短 package publish smoke：用临时 Git 仓库、临时 library/app project fixture 和 fake `dotnet.ps1` 固定默认库包边界、dry-run JSON、`pack` / `nuget push` 参数、API key redaction、子进程输出预览脱敏、应用项目 warning 和 dirty worktree 下 `-Apply` 阻断行为。它不触碰真实 NuGet source，也不创建真实 package registry 发布。

`verify-agent-package-consumer.ps1` 是 Agent 底座的本地 NuGet consumer smoke：它先把当前 `Tau.Ai` 与 `Tau.Agent` 打包到临时本地 package source，检查 `Tau.Agent.nuspec` 对 `Tau.Ai` 的传递依赖、包 README 和描述元数据。随后脚本创建两个临时外部 console app：第一个只 `PackageReference` `Tau.Ai`，通过 `Tau.Ai.Providers.Faux`、`ProviderRegistry` 和 `StreamFunctions` 完成一次 LLM 调用；第二个只 `PackageReference` `Tau.Agent`，通过传递依赖使用 `Tau.Ai.Providers.Faux`，运行 `AgentApplication` + delegate tool + session/log 回合并断言 `assistant=package consumer complete`、4 条 message、1 个 tool start/end、2 次 provider run、session snapshot 和 faux call count。它证明的是本地包消费边界，不触碰真实 NuGet source，也不等于真实 registry 发布演练。
...
`verify-release-contracts.ps1` 是短 release script contract smoke，不跑完整 `dotnet build/test`，不生成 artifact，也不写版本或 release notes。它调用 `plan-release.ps1`、`prepare-release.ps1`、`validate-release.ps1` 和 `execute-release.ps1` 的 `-Json` dry-run 输出，并调用 `verify-release-finalize.ps1 -Json`、`verify-release-package-publish.ps1 -Json`、`verify-ai-cli-tool-install.ps1 -SkipRestore -Json`、`verify-agent-package-consumer.ps1 -SkipRestore -Json`、`verify-agent-proxy-server-e2e.ps1 -SkipRestore -Json`、`verify-release-provenance.ps1 -Json` 与 `verify-ai-test-image.ps1 -Json` 固定 finalization / package publish / AI CLI tool install / package consumer / agent proxy server e2e / provenance signing / AI test image 合同，断言 dry-run boundary、next version、planned command names、local release mutation plan、guarded finalize smoke、guarded package publish smoke、AI CLI tool install smoke、agent package consumer smoke、agent proxy server e2e smoke、guarded provenance/signing smoke、AI test image smoke、non-executed package publish/signing/provenance/push/external mutation boundary、preparation changed files，以及 validation coverage metadata。这个脚本用于固定 release automation 的 JSON 契约，当前 CI 会在长 no-env/build/release artifact 步骤前先运行它。
