...
- 已有：
  ...
  - `Tau.CodingAgent`：最小可运行 CLI、基础文件/命令工具、和 `ModelCatalog` 对齐的默认 provider / model 接线、显式 `Create(provider, model, history)` runner 工厂、JSONL session tree baseline（`/session` token/context budget、`/tree`、`/label`、`/fork`、`/resume`、`.jsonl` export/import、手动/自动 `compaction` entry metadata）、prompt template discovery/expansion、skill command discovery/expansion、JSON 声明式 extension command 与 prompt/skill resource discovery baseline，以及带内嵌 JSONL 下载的 `.html/.htm` transcript export
...
- `Tau.CodingAgent` 已有 flat JSON session 和 JSONL tree session 双路径；`/session` 会显示估算 token、模型 context window 和 auto-compaction threshold budget；`/tree`、`/label`、`/fork`、`/resume`、`/compact`、自动 compaction threshold、`/prompts`、`/skills`、`/skill:<name>`、`/extensions`、JSON extension command、`/export [path]` 可通过 targeted test 或真实 CLI smoke 运行
...
- `Tau.CodingAgent.Tests` 已通过 79 个测试
