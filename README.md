...
- `Tau.WebUi` 已从 Hello World 推进到 **可持久化 session + provider/model 选择 + 流式/附件/会话管理 + JSON/HTML/Markdown/JSONL 导入导出 + CodingAgent JSONL 只读预览/保守导入** 的 Web 宿主
...
- 未完成：
  ...
  - Pods 的真实 vLLM orchestration、更完整模型生命周期、更完整远端 transport hardening 和真实运维 smoke；当前只有 command planner 与 `vllm plan` 预览入口
  - 更高层行为回归测试
...
- `vllm plan`
  ...
  - 输出 deployment/modelPath/port/servedModel/systemd unit/metadata JSON/remote command
...
- `Tau.CodingAgent.Tests` 已通过 342 个测试
- `Tau.WebUi.Tests` 已通过 29 个测试
- `Tau.Agent.Tests` 已通过 76 个 Mom/Slack channel runtime 测试
- `Tau.Pods.Tests` 已通过 65 个测试
- `Tau.Tui.Tests` 已通过 110 个测试
- `Tau.Ai.Tests` 已通过 204 个测试
