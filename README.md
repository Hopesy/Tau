...
- 已有：
  ...
  - `Tau.CodingAgent`：最小可运行 CLI、基础文件/命令工具、和 `ModelCatalog` 对齐的默认 provider / model 接线、显式 `Create(provider, model, history)` runner 工厂、JSONL session tree baseline（`/session` token/context budget、`/tree` filter/search modes、`cwd` / `parentSession` metadata、settings `treeFilterMode` / label timestamp、`/label`、`/fork`、`/clone`、`/resume`、`.jsonl` export/import、手动/自动 `compaction` entry metadata）、prompt template discovery/expansion、skill command discovery/expansion、JSON 声明式 extension command 与 prompt/skill resource discovery baseline，以及带 cwd/parent metadata 和内嵌 JSONL 下载的 `.html/.htm` transcript export
...
- `Tau.CodingAgent` 已有 flat JSON session 和 JSONL tree session 双路径；`/session` 会显示估算 token、模型 context window、auto-compaction threshold budget、JSONL `cwd` 和 clone/export 产生的 `parentSession`；`/tree [max entries] [default|no-tools|user-only|labeled-only|all] [--label-time] [--search query]` 会读取 settings `treeFilterMode` 作为默认过滤模式，并在 header 中显示 `cwd` / `parent` metadata；HTML transcript export 也会显示 tree cwd / parent metadata；`/label`、`/fork`、`/clone`、`/resume`、`/compact`、自动 compaction threshold、`/prompts`、`/skills`、`/skill:<name>`、`/extensions`、JSON extension command、`/export [path]` 可通过 targeted test 或真实 CLI smoke 运行
