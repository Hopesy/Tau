...
- 但本机 bash 服务可能报 `Bash/Service/CreateInstance/E_ACCESSDENIED`
- 所以现场验证可以退回等价顺序的 `dotnet build/test/run` 命令
...
- `Tau.CodingAgent.Tests` 已通过 6 个测试
- `Tau.Agent.Tests` 已通过 2 个 Mom 结构化委派测试
- `Tau.Pods.Tests` 已通过 7 个测试
- `Tau.Ai.Tests` 已通过 provider serialization 回归测试链
