...
- sample pod config；未设置 `PI_CONFIG_DIR` 时默认 `tau.pods.json`，设置后默认 `<PI_CONFIG_DIR>/pods.json`
...
- `deploy / stop / restart`
  ...
  - 对 endpoint pod 明确返回 unsupported
  - `<path>` 可省略；`deploy gpu-1 model-id` 未设置 `PI_CONFIG_DIR` 时使用默认 `tau.pods.json`，设置后使用 `<PI_CONFIG_DIR>/pods.json`；`deploy custom.json gpu-1 model-id` 使用显式配置文件并优先于 env default
...
`verify-release-contracts.ps1` 是短 release script contract smoke，不跑 `dotnet build/test`，不生成 artifact，也不写版本或 release notes。它调用 `plan-release.ps1`、`prepare-release.ps1`、`validate-release.ps1` 和 `execute-release.ps1` 的 `-Json` dry-run 输出，并调用 `verify-release-finalize.ps1 -Json`、`verify-release-package-publish.ps1 -Json`、`verify-release-provenance.ps1 -Json` 与 `verify-ai-test-image.ps1 -Json` 固定 finalization / package publish / provenance signing / AI test image 合同，断言 dry-run boundary、next version、planned command names、local release mutation plan、guarded finalize smoke、guarded package publish smoke、guarded provenance/signing smoke、AI test image smoke、non-executed package publish/signing/provenance/push/external mutation boundary、preparation changed files，以及 validation coverage metadata。这个脚本用于固定 release automation 的 JSON 契约，当前 CI 会在长 no-env/build/release artifact 步骤前先运行它。
