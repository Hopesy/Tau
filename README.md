...
`verify-release-contracts.ps1` 是短 release script contract smoke，不跑 `dotnet build/test`，不生成 artifact，也不写版本或 release notes。它调用 `plan-release.ps1`、`prepare-release.ps1`、`validate-release.ps1` 和 `execute-release.ps1` 的 `-Json` dry-run 输出，并调用 `verify-release-finalize.ps1 -Json` 与 `verify-release-package-publish.ps1 -Json` 固定 finalization / package publish 合同，断言 dry-run boundary、next version、planned command names、local release mutation plan、guarded finalize smoke、guarded package publish smoke、non-executed package publish/push/external mutation boundary、preparation changed files，以及 validation coverage metadata。这个脚本用于固定 release automation 的 JSON 契约，当前 CI 会在长 no-env/build/release artifact 步骤前先运行它。
