...
- `Tau.Pods` 已从静态 config CLI 推进到 **支持 probe / exec / health / deploy / stop / restart / model lifecycle / vLLM serve command planner 与 `vllm plan/deploy/status/health/stop` CLI baseline、vLLM health retry/backoff 且 SSH exec 走 ArgumentList argv 构造并结构化处理本地 ssh 进程失败** 的运维 CLI
...
- 已有：
  ...
  - `Tau.Pods`：`init / list / validate / status / probe / exec / health / deploy / stop / restart / model list / model pull / model remove / model status / vllm plan / vllm deploy / vllm status / vllm health / vllm stop` 命令、sample config、validator、主动 endpoint/tcp 探测、SSH 远程命令执行、SSH lifecycle metadata 管理、SSH exec `ArgumentList` argv hardening、长耗时 model pull keepalive、vLLM serve command planner 与 SSH orchestration baseline、本地 ssh 启动失败/runner 异常/cancellation 结构化返回、AOT 友好的 JSON source-gen
- 未完成：
...
- `vllm deploy / status / health / stop`
  - 只支持 SSH pod；endpoint pod 明确返回 unsupported
  ...
  - 当前是 SSH command/service orchestration baseline，尚未做真实 GPU pod、HF snapshot、vLLM 启动 smoke 或多版本 rollout smoke
...
dotnet run --project src/Tau.Pods/Tau.Pods.csproj --no-build -- vllm deploy --json tau.pods.json <pod-id> <model-id> [deployment-name]
