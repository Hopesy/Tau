...
- 已有：
  - `Tau.Ai`：消息抽象、流事件、EventStream、provider 注册、model catalog、OAuth / env auth 解析、`auth.json` / `models.json` 本地配置和 secret 状态边界；`models.json` 里显式标记为 OpenAI-compatible 的未知 API 可在运行时注册到 provider registry，公开上游 API 名 `openai-completions` / `openai-compatible` / `google-generative-ai` 会在 registry/config 层规范化到 Tau canonical `openai-chat-completions` / `google-generative-language`；`models.json` 的 provider-level、`modelOverrides.<model>.options` 和 `models[].options` 可配置通用 request options：`temperature`、`maxTokens`、`topP`、`transport`、`cacheRetention`、`sessionId`、`maxRetryDelayMs`、`metadata`、`reasoning` 和 `thinkingBudgets`，并已覆盖 OpenAI chat-completions provider-specific options：`openai-chat-completions` 的 `toolChoice`（字符串或 function 对象）和 `reasoningEffort`，Responses 家族第一批 provider-specific options：`openai-responses` 的 `serviceTier`、`reasoningEffort`、`reasoningSummary`，`openai-codex-responses` 的 `textVerbosity`，`azure-openai-responses` 的 `azureApiVersion`、`azureResourceName`、`azureBaseUrl`、`azureDeploymentName`，Mistral provider-specific options：`mistral-conversations` 的字符串型 `toolChoice`、对象型 function `toolChoice`、`promptMode`、`reasoningEffort`，Anthropic provider-specific options：`anthropic-messages` 的 `thinkingEnabled`、`thinkingBudgetTokens`、adaptive `effort`、`thinkingDisplay`、`interleavedThinking` 和字符串/tool 对象型 `toolChoice`，Google provider-specific options：`google-generative-language` 的 `toolChoice`、`thinkingEnabled`、`thinkingBudgetTokens`，`google-vertex` 的 `project/location`，`google-gemini-cli` 的 `thinkingLevel` 和 `projectId`，以及 Bedrock provider-specific options：`bedrock-converse-stream` 的 `region`、`profile`、`bearerToken`、字符串/tool 对象型 `toolChoice`、`thinkingBudgetTokens`、`thinkingDisplay`、`interleavedThinking` 和 `requestMetadata`；代码显式参数优先，`Signal` / `OnPayload` / `OnResponse` 仍只来自代码；Bedrock SSO cache token 过期时可在 AWS CLI cache 含 `clientId` / `clientSecret` / `refreshToken` / 未过期 `registrationExpiresAt` 时调用 OIDC `CreateToken` 刷新，并 best-effort 写回 cache；`JsonlTauLogSink` 写出的 Tau runtime event log 会先生成完整 JSONL，再复用 `JsonlSecretRedactor` 递归脱敏 JSON string value（category、event 和 field value），保留 field key / number / bool / null，可用 `TAU_LOG_REDACT_SECRETS=0` 在 sink 创建时显式关闭；`JsonlSecretRedactor` 提供通用 JSONL 行级 string value redaction foundation，当前已被 CodingAgent JSONL tree session 写出、WebUi JSONL export/import / CodingAgent JSONL preview-import 以及 Mom 的 `log.jsonl` / `last_prompt.jsonl` 脱敏复用；该能力保留 field key，不覆盖非标准 secret pattern，也不是通用 secret scanner
...
- sample pod config；未设置 `PI_CONFIG_DIR` 时默认 `~/.pi/pods.json`，设置后默认 `<PI_CONFIG_DIR>/pods.json`
...
- `deploy / stop / restart`
  ...
  - 对 endpoint pod 明确返回 unsupported
  - `<path>` 可省略；`deploy gpu-1 model-id` 未设置 `PI_CONFIG_DIR` 时使用默认 `~/.pi/pods.json`，设置后使用 `<PI_CONFIG_DIR>/pods.json`；`deploy custom.json gpu-1 model-id` 使用显式配置文件并优先于 env default
...
powershell -ExecutionPolicy Bypass -File .\scripts\finalize-release.ps1 v0.1.1
powershell -ExecutionPolicy Bypass -File .\scripts\publish-release-packages.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\generate-release-provenance.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\sign-release-packages.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-release-contracts.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-release-finalize.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-release-package-publish.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-ai-cli-tool-install.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-agent-package-consumer.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-agent-proxy-server-e2e.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-ai-provider-e2e-matrix.ps1 -Isolated
powershell -ExecutionPolicy Bypass -File .\scripts\verify-ai-agent-export-shape.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-release-provenance.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-ai-test-image.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-session-audit-scripts.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-coding-agent-session-migration.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-coding-agent-commands-migration.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-coding-agent-tools-to-bin-migration.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\verify-coding-agent-deprecated-extension-dirs-audit.ps1
...
`plan-release.ps1` 对照上游 `scripts/release.mjs` 的 clean worktree、version bump / explicit semver、changelog release section、commit/tag、publish 和 push 流程，生成 Tau 的 dry-run 发布计划。它会读取 `git status`、检查 release notes 与 release 脚本是否存在、从 `Directory.Build.props` 的 `VersionPrefix` 读取当前 Tau 产品版本、计算 `major|minor|patch` 或显式 `x.y.z` 的下一版本，并列出应运行的 guarded release preparation preview、guarded release validation preview、local release execution preview、guarded release finalization preview、guarded package publish preview、AI CLI tool install smoke、agent package consumer smoke、agent proxy-server e2e smoke、AI/Agent export-shape smoke、release provenance preview、release package signing preview、version update preview、release notes update preview、release contract smoke、release finalize smoke、release package publish smoke、release provenance/signing smoke、AI test image smoke、session audit script smoke、CodingAgent session migration smoke、CodingAgent commands migration smoke、CodingAgent tools-to-bin migration smoke、CodingAgent deprecated extension dirs audit smoke、edit tool stats smoke、Mom timestamp migration smoke、CodingAgent startup profile smoke、release version sync smoke、no-env gate、release matrix build/package 命令；脚本不会修改版本、release notes、history，不会执行 `git commit`、`git tag`、GitHub Release 创建、package registry publish、package signing、provenance 写入或 push。`-CurrentVersion x.y.z` 仅用于临时覆盖当前版本；`-AllowDirty` 只用于 planning-only 场景，真实 release 前仍要求 clean worktree。
...
`execute-release.ps1` 是当前本地 release execution 的受控执行层：默认 dry-run，只预览 release plan、preparation、validation 和 local mutation plan；只有显式传 `-Apply` 且开始时工作树干净，才会先运行 `verify-release-contracts.ps1`，再执行 `prepare-release.ps1 -Apply`，按需运行 `validate-release.ps1 -Run -AllowDirty`，随后只 stage `Directory.Build.props` 与 `docs/releases/feature-release-notes.md`，创建 `Release v<version>` commit，并打 `v<version>` tag。脚本会拒绝已有同名 tag，且会在 prepare 后检查没有其它意外文件变更；它仍不 publish、不 push，也不生成上游第二个 `Add [Unreleased] section` commit，因为 Tau release notes 是日期表格而不是每包 `CHANGELOG.md` section。后续远端发布收口交给 `finalize-release.ps1`，而不是在本地 version/notes commit 阶段隐式触碰远端。

`finalize-release.ps1` 是当前 release finalization 的受控远端层：默认 dry-run，只预览 `git push <remote> <branch>`、`git push <remote> <tag>`，以及可选的 GitHub Release archive upload；只有显式传 `-Apply`，并通过 release tag 格式、git 可用性、clean worktree、remote、local branch、local tag、tag 指向 branch tip、release archive 存在等预检后，才真正 push branch 和 tag。传 `-CreateGitHubRelease` 时会额外要求 release notes 与 `gh` CLI 可用，并调用 `gh release create <tag> <archives> --title ... --notes-file ...` 上传已验证 archive；dry-run 预览与实际执行都会保留 `-Draft` / `-Prerelease` 对应的 `--draft` / `--prerelease` flag。这只覆盖 GitHub Release archive upload，不等于 NuGet/package registry publish synchronization。`-AllowDirty` 仅用于显式放宽本地 worktree 检查，`-SkipArchiveCheck` 仅用于受控场景绕过 archive 预检。

`verify-release-finalize.ps1` 是短 release finalization smoke：用临时 Git 仓库、临时 bare remote、fake release archive 和 fake `gh.cmd` 固定 dry-run JSON contract、`-Apply` push branch/tag、GitHub Release create 参数、draft/prerelease flag 预览与执行一致性，以及 dirty worktree 下 `-Apply` 阻断行为。它不触碰真实 remote，不创建真实 GitHub Release，也不清理临时 fixture root，避免 Windows 文件锁导致验证卡住。

`publish-release-packages.ps1` 是当前 package registry publish synchronization 的受控层：默认 dry-run，只预览 `dotnet pack` 与 `dotnet nuget push`；显式 `-Apply` 才会先打包再推送。默认项目边界覆盖上游 workspace public library 对应的 `Tau.Ai`、`Tau.Agent`、`Tau.Tui` 三个库包，并把 `Tau.Ai.Cli` 作为两个 dotnet tool package 规格一起回放：`Tau.Ai.Cli.PiAiTool` / `ToolCommandName=pi-ai` 和 `Tau.Ai.Cli.TauAiTool` / `ToolCommandName=tau-ai`。应用项目仍默认通过 release archives 交付，只有显式 `-ProjectPath` 才会纳入并给出 application package warning。脚本从 `Directory.Build.props` 读取版本，默认输出到 `artifacts/nuget`，默认推送到 NuGet v3 source，并只从 `-ApiKeyEnv` 指定的环境变量读取 API key；JSON、命令预览和子进程输出预览只报告 env 名称与 present 状态，不回显密钥。真实发布必须显式 `-Apply`，并确认 package source、凭证、library/tool/application 包边界和 promotion 策略；`-SkipToolPackages` 可跳过 AI CLI tool package 回放，`-SkipPush` 可只打包，`-PackagePath` / `-SkipPack` 可推送已存在包，`-SkipDuplicate` 对应 `dotnet nuget push --skip-duplicate`。

`verify-release-package-publish.ps1` 是短 package publish smoke：用临时 Git 仓库、临时 library/tool/app project fixture 和 fake `dotnet.ps1` 固定默认 3 个库包 + 2 个 AI CLI tool 包边界、dry-run JSON、`pack` / `nuget push` 参数、`PackAsTool=true`、`ToolCommandName=pi-ai|tau-ai`、API key redaction、子进程输出预览脱敏、应用项目 warning 和 dirty worktree 下 `-Apply` 阻断行为。它不触碰真实 NuGet source，也不创建真实 package registry 发布。

`verify-agent-package-consumer.ps1` 是 Agent 底座的本地 NuGet consumer smoke：它先把当前 `Tau.Ai` 与 `Tau.Agent` 打包到临时本地 package source，检查 `Tau.Agent.nuspec` 对 `Tau.Ai` 的传递依赖、包 README 和描述元数据。随后脚本创建两个临时外部 console app：第一个只 `PackageReference` `Tau.Ai`，通过 `Tau.Ai.Providers.Faux`、`ProviderRegistry` 和 `StreamFunctions` 完成一次 LLM 调用，并额外证明显式 `ModelConfigurationStore` / `ProviderAuthResolver` 注入后，`models.json` 动态 provider、`auth.json`、model-level/provider-level auth status、`Authorization` header、provider/model/explicit header 覆盖链路、上游 `google-generative-ai` API 名 alias 调度到 Tau canonical `google-generative-language` provider、provider/model/default `models.json options` 注入、OpenAI chat-completions provider-specific options typed dispatch、OpenAI Responses provider-specific options typed dispatch、Mistral provider-specific options typed dispatch（含 function `toolChoice` 对象）、Anthropic provider-specific options typed dispatch（thinking、display、interleaved thinking 和 tool `toolChoice`）、Google provider-specific options typed dispatch（tool choice、thinking budget 和 max tokens），以及 Bedrock provider-specific options typed dispatch（region/profile/bearer token、tool choice、thinking display/interleaved thinking 和 request metadata）都能在外部 consumer 中跑通；第二个只 `PackageReference` `Tau.Agent`，通过传递依赖使用 `Tau.Ai.Providers.Faux`，运行 `AgentApplication` + delegate tool + session/log 回合并断言 `assistant=package consumer complete`、4 条 message、1 个 tool start/end、2 次 provider run、session snapshot 和 faux call count。当前 smoke 为 91 assertions。它证明的是本地包消费边界，不触碰真实 NuGet source，也不等于真实 registry 发布演练。

`verify-ai-cli-tool-install.ps1` 是 `Tau.Ai.Cli` 的本地 dotnet tool install rehearsal：它把同一个 CLI 项目临时打成 `pi-ai` 和 `tau-ai` 两个 tool package，写入临时本地 package source，再安装到临时 `tool-path`，验证安装 shim 存在、`pi-ai --help` 显示 `Usage: pi-ai ...`、`pi-ai list` 输出 OAuth provider 列表、`tau-ai --help` 显示 `Usage: tau-ai ...`。它证明的是本地 tool package/install alias 行为，不触碰真实 NuGet source，不做真实 signing/provenance，也不等于最终 registry/global install 完成。

`verify-agent-proxy-server-e2e.ps1` 是 Agent proxy 的本地 server-path smoke：它运行 `ProxyStreamProviderTests`，其中包含一个真实 loopback TCP HTTP/SSE server。测试会让 `ProxyStreamProvider` 向 `/api/stream` 发起真实 POST，校验 `Authorization: Bearer ...`、JSON request envelope 和 SSE response 重建，同时覆盖 HTTP error、缺 terminal event 和 malformed SSE JSON 的 terminal error 行为。它证明的是 Tau.Agent proxy provider 的本地 server-path 合同，不触碰真实 provider 凭证，也不等于真实 provider/OAuth e2e。

`verify-ai-provider-e2e-matrix.ps1` 是 AI provider/OAuth 的可审计矩阵脚本：默认 `inspect` 只读 provider/auth/model 状态，`-Isolated` 会把 `TAU_AUTH_FILE` 与 `TAU_MODELS_FILE` 指到临时空文件并清空常见 provider env，`-RunConfigured` 则只对已配置凭证的 provider 发起最小真实调用。需要把结果作为真实 provider/OAuth e2e 验收证据时必须加 `-RequireConfigured`：没有任何已配置 provider、没有真实 run attempt 或非隔离全成功结果时脚本会非零退出，并在 JSON 中标记 `completionStatus` / `gateFailure`。默认 provider 列表覆盖 `openai`、`openai-codex`、`azure-openai-responses`、`github-copilot`、`anthropic`、`google`、`google-vertex`、`google-gemini-cli`、`google-antigravity`、`mistral` 和 `amazon-bedrock`。它证明的是本地 provider/OAuth 检查与可执行矩阵契约；只有带真实凭证的 `-RunConfigured -RequireConfigured` 非隔离成功结果才能作为真实 cloud e2e 证据。

`verify-ai-agent-export-shape.ps1` 是 AI/Agent public export shape 的本地决策 smoke：它读取 `docs/AI_AGENT_EXPORT_SHAPE.md`，断言上游 `packages/ai/package.json` 的 12 个 exports、`packages/ai/src/index.ts` 的 13 组 public re-export 和 `packages/agent/src/index.ts` 的 4 组 re-export 都有 Tau-native 映射，并检查对应实现文件、public API compile sample、package consumer、AI CLI tool install、package publish 和 Agent proxy smoke 入口仍存在。它证明的是 Tau 选择 .NET assembly / namespace / NuGet package surface，而不是新增 TypeScript/npm compatibility shim；真实 provider/OAuth、真实 registry/global install、签名和 provenance 仍是独立 gate。

`generate-release-provenance.ps1` 是当前 release provenance manifest 的受控层：默认 dry-run，扫描 `artifacts/releases/*.zip` / `*.tar.gz` 和 `artifacts/nuget/*.nupkg`（排除 `.symbols.nupkg`），记录 `schemaVersion`、生成时间、git commit/branch/worktree 状态、MSBuild version source、每个 release archive / NuGet package 的相对路径、文件名、格式、size 和 SHA256。显式 `-Apply` 才会写入 `artifacts/provenance/release-provenance.json`，且默认要求 clean worktree；`-ArtifactPath` / `-PackagePath` 可指定已存在制品。该 manifest 不签名，不发布 registry，也不等于外部供应链 attestation。

`sign-release-packages.ps1` 是当前 NuGet package signing 的受控层：默认 dry-run，只预览 `dotnet nuget sign`；显式 `-Apply` 才会执行。脚本要求 `-CertificatePath` 或 `-CertificateFingerprint` 二选一，支持 `-TimestampUrl`、`-HashAlgorithm SHA256|SHA384|SHA512`、`-TimestampHashAlgorithm`、`-OutputDirectory`、`-Overwrite` 和 `-DotnetCli`；证书密码只从 `-CertificatePasswordEnv` 指定的环境变量读取，JSON、命令预览和子进程输出预览都会脱敏。该脚本按 .NET CLI 的 NuGet signing 入口实现，只覆盖 NuGet 包签名，不签 release archive，也不做真实证书有效性或签名验证。

`verify-release-provenance.ps1` 是短 provenance/signing smoke：用临时 Git 仓库、release archive / NuGet package fixture 和 fake `dotnet.ps1` 固定 provenance JSON、SHA256/size/path、symbols 包排除、dirty worktree 下 provenance `-Apply` 阻断、`dotnet nuget sign` dry-run command shape、证书路径/指纹入口、timestamp 参数、fake signing apply、无证书 `-Apply` 阻断，以及证书密码在命令/输出/JSON 中不泄漏。它不使用真实 code-signing certificate，也不触碰真实 package registry。

`generate-ai-test-image.ps1` 对照上游 `packages/ai/scripts/generate-test-image.ts`，生成 200x200 白底红圆 PNG fixture，默认输出到 `tests/Tau.Ai.Tests/Data/red-circle.png`，也支持 `-OutputPath` 指向临时路径。它用纯 PowerShell 写 PNG/IHDR/IDAT/IEND 和 zlib stored blocks，不依赖 node-canvas 或 System.Drawing。`verify-ai-test-image.ps1` 会在临时目录生成图片，解析 PNG chunk、zlib/adler32、scanline 和关键像素，固定 width/height/radius、中心红色、背景白色和 SHA256 输出。

`verify-release-contracts.ps1` 是短 release script contract smoke，不跑完整 `dotnet build/test`，不生成 artifact，也不写版本或 release notes。它调用 `plan-release.ps1`、`prepare-release.ps1`、`validate-release.ps1` 和 `execute-release.ps1` 的 `-Json` dry-run 输出，并调用 `verify-release-finalize.ps1 -Json`、`verify-release-package-publish.ps1 -Json`、`verify-ai-cli-tool-install.ps1 -SkipRestore -Json`、`verify-agent-package-consumer.ps1 -SkipRestore -Json`、`verify-agent-proxy-server-e2e.ps1 -SkipRestore -Json`、`verify-ai-provider-e2e-matrix.ps1 -Isolated -Json`、`verify-ai-provider-e2e-matrix.ps1 -RunConfigured -RequireConfigured -Isolated -Json`、`verify-ai-agent-export-shape.ps1 -Json`、`verify-release-provenance.ps1 -Json` 与 `verify-ai-test-image.ps1 -Json` 固定 finalization / package publish / AI CLI tool install / package consumer / agent proxy server e2e / AI provider/OAuth matrix / AI-Agent export shape / provenance signing / AI test image 合同，断言 dry-run boundary、next version、planned command names、local release mutation plan、guarded finalize smoke、guarded package publish smoke、AI CLI tool install smoke、agent package consumer smoke、agent proxy server e2e smoke、AI provider/OAuth matrix smoke、required real-e2e guard 在无凭证隔离环境下必须阻断、AI/Agent export shape smoke、guarded provenance/signing smoke、AI test image smoke、non-executed package publish/signing/provenance/push/external mutation boundary、preparation changed files，以及 validation coverage metadata。这个脚本用于固定 release automation 的 JSON 契约，当前 CI 会在长 no-env/build/release artifact 步骤前先运行它。
...
powershell -ExecutionPolicy Bypass -File .\scripts\migrate-coding-agent-sessions.ps1 -AgentDirectory .\.tau
powershell -ExecutionPolicy Bypass -File .\scripts\migrate-coding-agent-commands.ps1 -BaseDirectory .\.tau
powershell -ExecutionPolicy Bypass -File .\scripts\migrate-coding-agent-tools-to-bin.ps1 -AgentDirectory .\.tau
powershell -ExecutionPolicy Bypass -File .\scripts\audit-coding-agent-deprecated-extension-dirs.ps1 -BaseDirectory .\.tau -Label Project
...
`migrate-coding-agent-sessions.ps1` 对照上游 `packages/coding-agent/scripts/migrate-sessions.sh` 和 `migrations.ts` 的 root JSONL relocation 职责：扫描 `-AgentDirectory` 根目录直接子 `.jsonl`，读取第一行 `type=session` header 的 `cwd`，用上游两边包 `--`、并把 `/`、`\`、`:` 替换为 `-` 的编码规则计算目录，再把 misplaced root session 移到 Tau 当前 `/clone` / `/resume` 可发现的 `coding-agent-sessions/<encoded-cwd>/<filename>`。脚本默认 dry-run，只有显式 `-Apply` 才创建目录并移动文件；目标已存在、坏 JSON、无 `cwd`、非 session header 和 nested JSONL 会跳过。`-Json` 输出 scan/file summary。该脚本只关闭 Tau-native 可发现的 root JSONL relocation helper，不关闭上游 auth/settings/extensions/keybindings/tools migrations 或 byte-equivalent session schema migration。`verify-coding-agent-session-migration.ps1` 用临时 agent fixture 固定 dry-run 不移动、apply 移动、Windows cwd 编码、target conflict、坏 header 跳过、nested 不扫描和二次 apply 幂等。

`migrate-coding-agent-commands.ps1` 对照上游 `packages/coding-agent/src/migrations.ts` 的 `commands/` -> `prompts/` migration：默认检查用户 `~/.tau` 与当前项目 `./.tau`，也可用 `-BaseDirectory` 显式指定一个或多个 base directory；当 `commands/` 存在且 `prompts/` 不存在时把目录 rename 为 `prompts/`，支持普通目录和目录 symlink/reparse point。脚本默认 dry-run，只有显式 `-Apply` 才执行 rename；`prompts/` 已存在、没有 `commands/`、base directory 不存在或 `commands` 不是目录时会跳过；`-Json` 输出 base directory summary。该脚本只关闭 CodingAgent legacy commands-to-prompts migration helper，不关闭上游 auth/settings/session/keybindings/tools migrations、tools-to-bin migration、actual hooks/custom tools migration to extensions 或 general custom tool migration parity。`verify-coding-agent-commands-migration.ps1` 用临时 base directory fixture 固定 dry-run、apply、target-exists、no-commands、file source、missing base 和二次 apply 幂等。

`migrate-coding-agent-tools-to-bin.ps1` 对照上游 `packages/coding-agent/src/migrations.ts` 的 managed binaries `tools/` -> `bin/` migration：默认扫描用户 `~/.tau/tools` 下的 `fd`、`rg`、`fd.exe`、`rg.exe`，目标 `bin/` 不存在时创建并移动，目标文件已存在时删除旧 `tools/` 副本，非托管 custom tool 文件保持不动。脚本默认 dry-run，只有显式 `-Apply` 才 move/remove；source 不存在、`tools/` 不存在或 source 是目录时跳过；`-Json` 输出 binary summary。该脚本只关闭 managed fd/rg binary relocation helper，不关闭 auth/settings/session/keybindings migrations、actual hooks/custom tools migration to extensions 或 general custom tool migration parity。`verify-coding-agent-tools-to-bin-migration.ps1` 用临时 agent fixture 固定 dry-run、apply、duplicate removal、content preservation、custom tool preserved、directory source skip、missing tools dir 和二次 apply 幂等。

`audit-coding-agent-deprecated-extension-dirs.ps1` 对照上游 `packages/coding-agent/src/migrations.ts` 的 `checkDeprecatedExtensionDirs(...)` warning audit：默认检查用户 `~/.tau` 与当前项目 `./.tau`，也可用 `-BaseDirectory` / `-Label` 指定一个或多个 base directory；当 `hooks/` 存在时报告 hooks renamed warning，当 `tools/` 目录包含除 `fd`、`rg`、`fd.exe`、`rg.exe` 和隐藏 entry 以外的 custom entries 时报告 custom tools merged warning。脚本只审计不改文件；`-Json` 输出 warnings/directories summary，并包含上游 migration guide 与 extensions docs URL。该脚本只关闭 deprecated hooks/tools warning audit helper，不迁移 hooks 或 custom tools 到 `extensions/`，也不关闭 auth/settings/session/keybindings migrations 或 general custom tool migration parity。`verify-coding-agent-deprecated-extension-dirs-audit.ps1` 用临时 base directory fixture 固定 hooks warning、custom tools warning、managed binary ignore、hidden entry ignore、file tools path skip、missing base 和 no-mutation boundary。
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\profile-coding-agent-startup.ps1 -Mode rpc -Runs 5 -Warmup 1 -IsolatedAgentDir
powershell -ExecutionPolicy Bypass -File .\scripts\profile-coding-agent-startup.ps1 -Mode rpc -Runs 1 -Warmup 0 -IsolatedAgentDir -Json
powershell -ExecutionPolicy Bypass -File .\scripts\verify-coding-agent-startup-profile.ps1
```

`profile-coding-agent-startup.ps1` 对照上游 `scripts/profile-coding-agent-node.mjs` 的 RPC startup 计时职责，启动已构建的 `Tau.CodingAgent.dll` 并通过 `--mode rpc --no-context-files --no-themes` 发送 `get_state` JSONL，计时到成功 response。脚本默认在临时 Tau auth/models/session/settings/history/keybindings/log 路径下运行，并设置 offline / no-local-LLM 环境；`-IsolatedAgentDir` 会为本次 profile 创建隔离 agent dir。输出包含 min/median/average/max 和 `METRIC startup_time_ms=<median>`，`-Json` 会输出 machine-readable summary、measured/all runs、stdout/stderr bytes 和 remaining gaps。stdin 使用无 BOM JSONL 文件 + shell redirection，避免 Windows PowerShell/.NET `StandardInput` BOM 破坏 Tau RPC JSONL。当前只声明 RPC startup profiler；上游 TUI first-frame startup timing、CPU profile、Node/Bun runtime 对比和 `PI_STARTUP_BENCHMARK` 等价 hook 仍是显式剩余缺口。`verify-coding-agent-startup-profile.ps1` 是短 smoke，缺少 Debug assembly 时会先构建 `Tau.CodingAgent`，再固定 warmup/measured run shape、positive median、assembly path 和 TUI gap audit。
...
`sync-release-versions.ps1` 对照上游 `scripts/sync-versions.js` 的 lockstep version audit，但使用 Tau 的 .NET-native 版本模型：`Directory.Build.props` 是单一版本事实源，`src/**/*.csproj` 默认继承它，不维护 npm workspace dependency ranges。脚本会读取 `Directory.Build.props` 中唯一的 `Version` / `VersionPrefix` / `PackageVersion`，扫描 `src` 下项目的显式版本属性；dry-run 在发现显式项目版本漂移时返回非零，`-Apply` 会把这些显式项目版本同步到中心版本。`-IncludeTests` 可把 `tests` 项目也纳入审计。`verify-release-version-sync.ps1` 会先验证当前仓库无漂移，再用临时 fixture 固定 drift detection、ProjectReference 审计和 apply 修复行为。该脚本不替代 `update-release-version.ps1` 的版本 bump；NuGet/package publish synchronization 的 dry-run/apply 边界由 `publish-release-packages.ps1` 和 `verify-release-package-publish.ps1` 固定。
...
当前 CI 在 `push main`、`pull_request` 和手动触发时运行 Windows PowerShell gate：按 `global.json` 安装 .NET SDK，`dotnet restore Tau.slnx`，先执行 `verify-release-contracts.ps1` 固定 release dry-run JSON contract，再独立执行 `verify-release-finalize.ps1` 固定 release finalization temp-remote/fake-gh smoke，执行 `verify-release-package-publish.ps1` 固定 package publish temp-fixture/fake-dotnet smoke，并执行 `verify-release-provenance.ps1` 固定 provenance manifest 与 package signing temp-fixture/fake-dotnet smoke，执行 `verify-ai-test-image.ps1` 固定 AI test image fixture generator smoke，再执行 `verify-agent-proxy-server-e2e.ps1` 固定 Agent proxy loopback server-path smoke，执行 `verify-session-audit-scripts.ps1` 固定 session transcript/cost audit scripts 的 fixture smoke，执行 `verify-coding-agent-session-migration.ps1` 固定 CodingAgent root JSONL relocation fixture smoke，执行 `verify-coding-agent-commands-migration.ps1` 固定 CodingAgent commands-to-prompts fixture smoke，执行 `verify-coding-agent-tools-to-bin-migration.ps1` 固定 CodingAgent managed tools-to-bin fixture smoke，执行 `verify-coding-agent-deprecated-extension-dirs-audit.ps1` 固定 CodingAgent deprecated hooks/tools warning audit fixture smoke，执行 `verify-edit-tool-stats.ps1` 固定 edit tool stats fixture smoke，执行 `verify-mom-timestamp-migration.ps1` 固定 Mom timestamp migration fixture smoke，执行 `verify-coding-agent-startup-profile.ps1` 固定 CodingAgent RPC startup profiler smoke，并执行 `verify-release-version-sync.ps1` 固定 MSBuild release version sync smoke，随后执行 `verify-no-env.ps1 -SkipRestore -RunSmoke`，构建 Release artifact，通过 `package-release-matrix.ps1 -Runtimes win-x64` 打包 `artifacts/releases/tau-win-x64.zip`，再用 `package-release-artifacts.ps1 -ArchiveFormat tar.gz -SkipExecutableSmoke` 做 tar.gz 格式/解压结构 smoke，并先解压 smoke Windows zip 后再上传该 zip 作为 workflow artifact。该 workflow 复用仓库现有 PowerShell 脚本，不另建一套 CI-only 行为；它关闭的是 Windows current-RID CI/release artifact baseline、release dry-run contract smoke baseline、release finalize smoke baseline、release package publish smoke baseline、release provenance/signing smoke baseline、AI test image generator smoke baseline、Agent proxy loopback server-path smoke baseline、session audit script smoke baseline、CodingAgent session migration smoke baseline、CodingAgent commands migration smoke baseline、CodingAgent managed tools-to-bin migration smoke baseline、CodingAgent deprecated extension dirs audit smoke baseline、edit tool stats smoke baseline、Mom timestamp migration smoke baseline、CodingAgent RPC startup profile smoke baseline 和 release version sync smoke baseline，不代表非宿主平台 executable smoke、TUI first-frame profiling、Unix shell wrapper、真实 NuGet/package registry 发布演练、真实 package signing/provenance rehearsal 或真实外部 e2e release smoke 已完成。
...
- `Tau.Ai.Tests` 已通过 450 个测试
- 主验证链已经改成 `scripts/verify-dotnet.sh`，按显式项目顺序 restore / build / test
- Windows 本机已补 `scripts/verify-dotnet.ps1`，可完整覆盖同一组 restore / build / test 验证链，并可通过 `-RunSmoke` 额外执行 `tau-ai`、Agent examples、Agent package consumer、Agent proxy loopback server-path、`WebUi` 与 `Mom --once` 的最小运行态验证
